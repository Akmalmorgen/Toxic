===================== БЛОК 1 / 16 — ШАПКА, КОНФИГ =====================
"""
𐌽ꤕ𐌗ተ — анонимный Telegram-бот (aiogram v3, SQLite / PostgreSQL).
Сборка 2.0: Next.. (тизер/раскрытие/мэтч), глобальная «Отмена»,
VIP только за коины в магазине, /help и /menu раздельно.
"""
...
from aiogram.types import (
    ...
    BufferedInputFile,
    BotCommand,
    KeyboardButton as _AiKB,
    ReplyKeyboardMarkup as _AiRKM,
    InlineKeyboardButton as _AiIKB,
    InlineKeyboardMarkup as _AiIKM,
...
if not BOT_TOKEN or ":" not in BOT_TOKEN:
    ...
    raise SystemExit(1)
if not ADMIN_IDS:
    boot.warning("ADMIN_IDS пустой — админ-панель недоступна.")
...
def InlineKeyboardButton(text, **kw):
    return _AiIKB(text=text, **kw)
def InlineKeyboardMarkup(inline_keyboard, **kw):
    return _AiIKM(inline_keyboard=inline_keyboard, **kw)
    ...
    async def send_document(self, chat_id, document=None, **kw):
        if hasattr(document, "read"):
            ...
            fname = os.path.basename(
                getattr(document, "name", "") or "file.txt")
...
class _Msg:
    ...
    async def reply_text(self, text, reply_markup=None,
        ...
        if parse_mode is None and _needs_html(text):
            parse_mode = "HTML"
        return await self._m.answer(text, reply_markup=reply_markup,
...
NEXT_MIN_PROFILES = 100
NEXT_REPEAT_LIMIT = 3
NEXT_REPEAT_WINDOW_DAYS = 7
NEXT_NOTIF_TTL_DAYS = 14
REVEAL_FREE_PER_MONTH = 3
REVEAL_MAX_PACK = 5
REVEAL_PACKAGES = [
...
if USE_PG:
    ...
    class _PgCursor:
        def __init__(self, raw):
            self._raw = raw
            self._cols = [d[0] for d in raw.description] \
                if raw.description else []
            try:
    ...
    def _translate_sql_for_pg(sql):
        ...
        if re.search(r"\bINSERT\s+OR\s+IGNORE\b", q, re.I):
            q = re.sub(r"\bINSERT\s+OR\s+IGNORE\s+INTO\b",
                       "INSERT INTO", q, flags=re.I)
            if "on conflict" not in q.lower():
                q = q.rstrip().rstrip(";") + " ON CONFLICT DO NOTHING"
        if re.search(r"\bINSERT\s+OR\s+REPLACE\b", q, re.I):
            q = re.sub(r"\bINSERT\s+OR\s+REPLACE\s+INTO\b",
                       "INSERT INTO", q, flags=re.I)
        q = re.sub(r"strftime\('%s',\s*([^)]+)\)",
                   r"EXTRACT(EPOCH FROM \1)", q)
        return q
    class _PgConnection:
        ...
        def _connect(self):
            ...
            for attempt in range(1, 6):
                try:
                    self._conn = psycopg2.connect(
                        self.url, connect_timeout=15, keepalives=1,
            ...
            boot.critical("Не удалось подключиться к PostgreSQL: %s",
                          last_err)
            raise SystemExit(1)
    ...
    try:
        ...
        boot.warning("SQLite pragma: %s", e)
    boot.info("БД: SQLite (%s)", DB_PATH)
def db():
    return conn
===================== БЛОК 2 / 16 — СХЕМА БД, УТИЛИТЫ =====================
def init_db():
    cur = conn.cursor()
    cur.executescript("""
CREATE TABLE IF NOT EXISTS users (
    tg_id INTEGER PRIMARY KEY,
    username TEXT, first_name TEXT, gender TEXT, search_pref TEXT,
    custom_link TEXT UNIQUE, old_link TEXT, old_link_until TEXT,
    link_changed_at TEXT,
    coins INTEGER NOT NULL DEFAULT 0,
    vip_until TEXT, is_moder INTEGER NOT NULL DEFAULT 0,
    is_banned INTEGER NOT NULL DEFAULT 0, last_bonus TEXT,
    lang TEXT NOT NULL DEFAULT 'ru', moder_until TEXT,
    ...
    last_active TEXT, nudged_at TEXT,
    admin_unlocked INTEGER NOT NULL DEFAULT 0,
    age TEXT, ref_code TEXT, last_greet TEXT,
    created_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS anon_messages (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    from_id INTEGER NOT NULL, to_id INTEGER NOT NULL,
    msg_type TEXT NOT NULL, content_type TEXT NOT NULL,
    text TEXT, voice_file_id TEXT,
    answer_text TEXT, answer_voice_file_id TEXT,
    answered INTEGER NOT NULL DEFAULT 0,
    deleted INTEGER NOT NULL DEFAULT 0,
    parent_id INTEGER, owner_chat_message_id INTEGER,
    sender_chat_message_id INTEGER, created_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS reports (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    reporter_id INTEGER NOT NULL, reported_id INTEGER NOT NULL,
    context TEXT NOT NULL, reason TEXT, ref_id INTEGER,
    status TEXT NOT NULL DEFAULT 'pending', created_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS bans (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    owner_id INTEGER NOT NULL, banned_id INTEGER NOT NULL,
    until TEXT NOT NULL, created_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS roulette_queue (
    user_id INTEGER PRIMARY KEY, gender TEXT NOT NULL,
    pref TEXT NOT NULL, is_vip INTEGER NOT NULL DEFAULT 0,
    mode TEXT NOT NULL DEFAULT 'normal', joined_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS roulette_sessions (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user1_id INTEGER NOT NULL, user2_id INTEGER NOT NULL,
    active INTEGER NOT NULL DEFAULT 1, ended_by INTEGER,
...
CREATE TABLE IF NOT EXISTS shop_items (
    ...
    is_vip INTEGER NOT NULL DEFAULT 0, duration_days INTEGER,
...
CREATE TABLE IF NOT EXISTS mandatory_channels (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    chat_username TEXT NOT NULL, title TEXT, added_by INTEGER
);
CREATE TABLE IF NOT EXISTS moder_apps (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL, item_id INTEGER,
    price_paid INTEGER NOT NULL DEFAULT 0,
    gender TEXT, age TEXT, tg_time TEXT, availability TEXT,
    status TEXT NOT NULL DEFAULT 'pending', created_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS star_packages (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    title TEXT NOT NULL, title_uz TEXT, title_en TEXT,
    coins INTEGER NOT NULL, price_stars INTEGER NOT NULL,
    active INTEGER NOT NULL DEFAULT 1
);
CREATE TABLE IF NOT EXISTS star_purchases (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL, package_id INTEGER,
    coins INTEGER NOT NULL, stars INTEGER NOT NULL,
    charge_id TEXT, refunded INTEGER NOT NULL DEFAULT 0,
    refunded_at TEXT, created_at TEXT NOT NULL
);
...
CREATE TABLE IF NOT EXISTS link_flow (
    user_id INTEGER PRIMARY KEY, target_id INTEGER NOT NULL,
    msg_type TEXT, state TEXT NOT NULL, updated_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT);
CREATE TABLE IF NOT EXISTS nearby_profiles (
    user_id INTEGER PRIMARY KEY, name TEXT, age INTEGER,
    gender TEXT, looking_for TEXT, bio TEXT, photo_id TEXT,
    active INTEGER NOT NULL DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS nearby_matches (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user1_id INTEGER NOT NULL, user2_id INTEGER NOT NULL,
    created_at TEXT NOT NULL, UNIQUE(user1_id, user2_id)
);
CREATE TABLE IF NOT EXISTS nearby_messages (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    from_id INTEGER NOT NULL, to_id INTEGER NOT NULL,
    content_type TEXT NOT NULL, text TEXT, voice_file_id TEXT,
    parent_id INTEGER, owner_chat_message_id INTEGER,
...
CREATE TABLE IF NOT EXISTS nearby_answers (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    like_id INTEGER NOT NULL, user_id INTEGER NOT NULL,
    answer TEXT NOT NULL, created_at TEXT NOT NULL,
    UNIQUE(like_id, user_id)
);
CREATE TABLE IF NOT EXISTS nearby_shown (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    viewer_id INTEGER NOT NULL, target_id INTEGER NOT NULL,
    shown_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS nearby_notifications (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL, chat_id INTEGER NOT NULL,
    message_id INTEGER NOT NULL, type TEXT NOT NULL,
    target_id INTEGER, like_id INTEGER,
    expires_at TEXT NOT NULL, deleted INTEGER NOT NULL DEFAULT 0,
    created_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS nearby_reveals (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    viewer_id INTEGER NOT NULL, target_id INTEGER NOT NULL,
    created_at TEXT NOT NULL, UNIQUE(viewer_id, target_id)
);
CREATE TABLE IF NOT EXISTS reveal_paid (
    user_id INTEGER PRIMARY KEY,
    total INTEGER NOT NULL DEFAULT 0, used INTEGER NOT NULL DEFAULT 0
);
CREATE TABLE IF NOT EXISTS sex_rooms (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    title TEXT NOT NULL, owner_id INTEGER NOT NULL, created_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS sex_members (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    room_id INTEGER NOT NULL, user_id INTEGER NOT NULL,
    role TEXT NOT NULL, number INTEGER NOT NULL, joined_at TEXT NOT NULL,
    UNIQUE(room_id, user_id)
);
CREATE TABLE IF NOT EXISTS sex_messages (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    room_id INTEGER NOT NULL, sender_number INTEGER NOT NULL,
    content_type TEXT NOT NULL, text TEXT, voice_file_id TEXT,
    created_at TEXT NOT NULL
);
...
CREATE TABLE IF NOT EXISTS anon_watchers (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    mod_id INTEGER NOT NULL, target_id INTEGER NOT NULL,
    active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL,
    UNIQUE(mod_id, target_id)
);
CREATE TABLE IF NOT EXISTS reveal_purchases (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL, count INTEGER NOT NULL,
    stars INTEGER NOT NULL, charge_id TEXT,
    refunded INTEGER NOT NULL DEFAULT 0, refunded_at TEXT,
    created_at TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS reveal_free_used (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL, period TEXT NOT NULL,
    used INTEGER NOT NULL DEFAULT 0, UNIQUE(user_id, period)
);
""")
    conn.commit()
    migrate()
...
def set_setting(key, value):
    ...
    conn.commit()
def cfg_vip_days():
    return get_setting_int("ref_vip_days", REF_VIP_DAYS)
def cfg_vip_threshold():
    return max(1, get_setting_int("ref_vip_threshold", REF_VIP_THRESHOLD))
def cfg_moder_days():
    return get_setting_int("ref_moder_days", REF_MODER_DAYS)
def cfg_moder_threshold():
    return max(1, get_setting_int("ref_moder_threshold", REF_MODER_THRESHOLD))
def get_user(tg_id):
    return conn.execute(
...
def ensure_user(tg_id, username, first_name=None):
    ...
    if u["username"] != username:
        conn.execute("UPDATE users SET username=? WHERE tg_id=?",
                     (username, tg_id))
        changed = True
    if first_name and u["first_name"] != first_name:
        conn.execute("UPDATE users SET first_name=? WHERE tg_id=?",
                     (first_name, tg_id))
        changed = True
    if changed:
        conn.commit()
    ...
    try:
        ...
        if la:
            try:
                ...
                pass
        conn.execute("UPDATE users SET last_active=? WHERE tg_id=?",
                     (now_iso(), uid))
        conn.commit()
    except Exception as e:
...
def resolve_user_ref(text):
    ...
    if not uname:
        return None
    row = conn.execute(
        "SELECT tg_id FROM users WHERE LOWER(username)=?", (uname,)).fetchone()
    return row["tg_id"] if row else None
def is_admin(tg_id):
    return tg_id in ADMIN_IDS
def is_super_admin(tg_id):
...
def has_forbidden_contacts(text):
    ...
    if re.search(r"@[a-z][a-z0-9_]{2,}", low):
        return True
    if re.search(r"https?://|www\.|t\.me|telegram\.me|telegram\.dog|"
                 r"joinchat|tg://", low):
        return True
    if re.search(r"\b[a-z0-9][a-z0-9-]*\.(com|ru|uz|net|org|io|me|info|biz|tv|"
                 r"app|link|site|online|club|store|xyz|kz)\b", low):
        return True
    if re.search(r"\b(instagram|insta|tiktok|youtube|youtu|whatsapp|watsap|"
                 r"vatsap|facebook|snapchat|discord|onlyfans|vkontakte|"
                 r"тикток|инстаграм|ютуб|ватсап|вотсап)\b", low):
        return True
    for m in re.finditer(r"[\d\s\-()+]{10,}", text):
        if sum(c.isdigit() for c in m.group()) >= 10:
            return True
    return False
===================== БЛОК 3 / 16 — ЯЗЫКИ, BTN, CANON =====================
LANGS = ("ru", "uz", "en")
...
BTN = {
    ...
    "🛡 Помощь модера": ("🛡 Moderator yordami", "🛡 Moder help"),
    "🌐 Язык": ("🌐 Til", "🌐 Language"),
    ...
    "🚪 Выйти": ("🚪 Chiqish", "🚪 Exit"),
    "💬 Войти в комнату": ("💬 Xonaga kirish", "💬 Join room"),
}
...
_ALIAS["Next.."] = "🎯 𝐞𝐭.."
_ALIAS["Next"] = "🎯 𝐍𝐞𝐭.."
_ALIAS["Поблизости"] = "🎯 𝐞𝐭.."
_ALIAS["📍 Поблизости"] = "🎯 𝐞𝐭.."
_EMOJI_BUTTONS = {"❤️": "❤️", "❤": "❤️", "👎": "👎",
                  "💌": "💌", "⛔": "", "": "💤"}
    ...
    for row in markup.keyboard:
        ...
        for b in row:
            ...
            if lang == "ru":
                ...
                if pair:
                    new_row.append(
                        KeyboardButton(pair[0] if lang == "uz" else pair[1]))
                else:
                    new_row.append(KeyboardButton(txt))
        new_rows.append(new_row)
    return ReplyKeyboardMarkup(
        new_rows, resize_keyboard=markup.resize_keyboard,
...
T = {
    ...
    "next_ask_name": {
        "ru": "📝 Введи <b>имя</b> (2-30):",
        "uz": "📝 <b>Ismingizni</b> kiriting:",
        "en": "📝 Enter <b>name</b>:",
    },
    "next_ask_age": {
        "ru": "🎂 Сколько тебе <b>лет</b>? (12-99)",
        "uz": "🎂 <b>Yoshingiz</b>? (12-99)",
        "en": "🎂 How old? (12-99)",
    },
    "next_ask_gender": {
        "ru": "⚧ Твой <b>пол</b>:",
    ...
    "next_ask_looking": {
        "ru": "🎯 <b>Кого ищешь</b>?",
        "uz": "🎯 <b>Kimni qidirasiz</b>?",
        "en": "🎯 <b>Who are you looking for</b>?",
    },
    "next_ask_bio": {
        "ru": "📄 Расскажи <b>о себе</b> (5-200):",
        "uz": "📄 <b>O'zingiz haqida</b> (5-200):",
        "en": "📄 <b>About you</b> (5-200):",
    },
    "next_ask_photo": {
        "ru": "📷 Отправь <b>фото</b>:",
        "uz": "📷 <b>Foto</b> yuboring:",
        "en": "📷 Send <b>photo</b>:",
    },
    "next_name_invalid": {
        "ru": "❌ Имя 2-30 символов:",
        "uz": "❌ Ism 2-30 belgi:",
        "en": "❌ Name 2-30 chars:",
    },
    "next_age_invalid": {
    ...
    "next_search_stopped": {
        "ru": "💤 <b>Вышел из поиска</b>",
        "uz": "💤 <b>Qidiruvdan chiqdingiz</b>",
        "en": "💤 <b>Left the search</b>",
    },
    "next_match_title": {
        "ru": "💕 <b>Взаимно!</b>",
        "uz": "💕 <b>O'zaro!</b>",
    ...
    "next_msg_prompt": {
        "ru": "💌 Отправь текст или голосовое:",
        "uz": "💌 Matn yoki ovozli yuboring:",
        "en": "💌 Send text or voice:",
    },
    "next_msg_sent": {
        "ru": "✅ Отправлено",
        "uz": "✅ Yuborildi",
    ...
    "next_report_confirm": {
        "ru": "🚩 <b>Жалоба отправлена</b>",
        "uz": "🚩 <b>Shikoyat yuborildi</b>",
        "en": "🚩 <b>Report sent</b>",
    },
    "next_banned_user": {
        "ru": "🚫 Вы забанены",
    ...
    "next_only_text_voice": {
        "ru": "⚠️ Только текст или голосовое",
        "uz": "⚠️ Faqat matn yoki ovozli",
        "en": "⚠️ Text or voice only",
    },
    "next_teaser": {
        "ru": (
            "💕 <b>Ты понравилась {n} человеку(ам)!</b>\n"
            "Кто-то лайкнул твою анкету в 𝐍𝐞𝐱𝐭..\n"
            "Лайкни в ответ — и будет мэтч 🎉"
        ),
        "uz": (
            "💕 <b>Sizni {n} kishi yoqtirdi!</b>\n"
            "Kimdir 𝐍𝐞𝐱𝐭.. da anketingizga layk qoldirdi.\n"
            "Javob layk — va match bo'ladi 🎉"
        ),
        "en": (
            "💕 <b>{n} people liked you!</b>\n"
            "Someone liked your profile in 𝐍𝐞𝐱𝐭..\n"
            "Like back — and it's a match 🎉"
        ),
    },
    "auto_hello_url": {
        "ru": "👋 Привет! Я {name}, из 𝐍𝐞𝐱𝐭.. — хочу познакомиться 😊",
        "uz": "👋 Salom! Men {name}, 𝐍𝐞𝐭.. dan — tanishmoqchiman 😊",
        "en": "👋 Hi! I'm {name}, from 𝐍𝐞𝐱𝐭.. — let's chat 😊",
    },
    "match_goto_hint": {
        "ru": "👇 Кнопка ниже — сразу в ЛС, приветствие уже написано:",
        "uz": "👇 Pastdagi tugma — to'g'ridan-to'g'ri DM ga, salom yozilgan:",
        "en": "👇 Button below — straight to DM with a ready greeting:",
    },
    "reveal_left_btn": {
        "ru": "👁 Раскрыть · {n} ОСТАЛОСЬ",
        "uz": "👁 Aniqlash · {n} qoldi",
        "en": "👁 Reveal · {n} left",
    },
    "lang_choose": {
    ...
    "welcome": {
        "ru": (
            "🔥 <b>Привет, {name}!</b> 👋\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "Добро пожаловать в <b>ꤕተ</b> 🕶\n\n"
            "<blockquote>"
            "🔗 Анонимки по своей ссылке\n"
            "🎲 Чат-рулетка\n"
            "🎯 𝐍𝐞𝐱𝐭.. — мэтчи\n"
            "💎 VIP и бонусы"
            "</blockquote>\n\n"
    ...
    "help": {
        "ru": (
            "ℹ️ <b>ПОМОЩЬ 𐌽𐌗ተ</b>\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "🔗 <b>МОЯ ССЫЛКА</b> — личная ссылка для анонимок.\n"
            "• Вопрос или валентинка: текст, голос.\n"
            "• Premium: фото, стикеры, гиф, видео.\n"
            "• Ответ, жалоба, раскрыть отправителя, удалить у обоих.\n"
            "• Смена кода: раз в 3 дня, Premium — без лимита.\n"
            "• Лимит: 20 анонимок/сутки (Premium — без лимита).\n"
            "🎲 <b>РУЛЕТКА</b> — случайный собеседник.\n"
            "• Выбор кого искать, VIP — приоритет в очереди.\n"
            "• «Далее» — новый, «Стоп» — выход, жалоба.\n"
            "🎯 <b>𝐍𝐞𝐱𝐭..</b> — анкеты и мэтчи.\n"
            "• Анкета: имя, возраст, пол, кого ищу, о себе, фото.\n"
            "• Просмотр: лайк, дизлайк, написать, жалоба.\n"
            "• 👁 Раскрыть под карточкой — кто это, ДО лайка.\n"
            "• Уведомление «ты понравился» — со счётчиком.\n"
            "• Взаимный лайк = МЭТЧ: контакт бесплатно,\n"
            "  кнопка сразу в ЛС с готовым приветствием.\n"
            "👤 <b>ПРОФИЛЬ</b> — статистика, пол/возраст, подарок коинов.\n"
            "🛒 <b>МАГАЗИН (за коины)</b> — VIP 7/30 дней, коины,\n"
            "  заявка на модера. Скидка VIP −20%.\n"
            "👑 <b>БОНУСЫ VIP</b>:\n"
            "• 🎁 +5 коинов каждый день (daily).\n"
            "• 💎 −20% в магазине.\n"
            "• 👁 3 «Раскрыть» в месяц бесплатно.\n"
            "• 🚀 Приоритет в 𝐍𝐞𝐱𝐭.. и рулетке.\n"
            "• 🔗 Смена ссылки без кулдауна.\n"
            "• 🚫 Без рекламы + медиа в анонимках.\n"
            "⭐ <b>PREMIUM</b> — покупается ТОЛЬКО в 🛒 Магазине за коины!\n"
            "👥 <b>ПРИГЛАСИТЬ</b> — +50 коинов за друга (Premium +100).\n"
            "• Друг по ссылке VIP-а: +100/+200 ему.\n"
            "• Награды: VIP за 5 друзей, модер за 10.\n"
            "💎 <b>КОИНЫ</b> — пакеты за Stars.\n"
            "👁 <b>РАСКРЫТИЕ</b> — VIP: 3 бесп./мес, дальше 1⭐ или паки.\n"
            "🌐 <b>ЯЗЫК</b> — ru/uz/en.\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "🏠 Меню — кнопкой ниже"
        ),
        "uz": (
            "ℹ️ <b>YORDAM 𐌽𐌗ተ</b>\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "🔗 <b>HAVOLAM</b> — shaxsiy havola, anonim xabarlar.\n"
            "• Savol yoki valentinka: matn, ovoz.\n"
            "• Premium: foto, stiker, gif, video.\n"
            "• Javob, shikoyat, ochish, o'chirish.\n"
            "• Kod: 3 kunda 1 marta, Premium — cheksiz.\n"
            "• Limit: 20/kun (Premium — cheksiz).\n"
            "🎲 <b>CHAT-RULETKA</b> — tasodifiy suhbatdosh.\n"
            "• Kimni qidirish, VIP — ustuvorlik.\n"
            "🎯 <b>𝐍𝐞𝐱𝐭..</b> — anketa va matchlar.\n"
            "• Anketa yarating va ko'ring.\n"
            "• 👁 Ochish — laykgacha kimligini bilish.\n"
            "• «Sizni yoqtirishdi» — hisoblagich bilan.\n"
            "• O'zaro layk = MATCH: kontakt bepul, DM ga tugma.\n"
            "👤 <b>PROFIL</b> — statistika, sovg'a coinlar.\n"
            "🛒 <b>DO'KON (coinlar)</b> — VIP 7/30 kun, coinlar, moder.\n"
            "👑 <b>VIP BONUSLAR</b>:\n"
            "• 🎁 Kuniga +5 coin.\n"
            "• 💎 Do'konda −20%.\n"
            "• 👁 Oyiga 3 «Aniqlash» bepul.\n"
            "• 🚀 Ustuvorlik.\n"
            "• 🔗 Havola cheklovsiz.\n"
            "• 🚫 Reklamasiz + media anonim.\n"
            "⭐ <b>PREMIUM</b> — faqat 🛒 Do'konda coinlar evaziga!\n"
            "👥 <b>TAKLIF</b> — do'st uchun +50 (Premium +100).\n"
            "• Mukofot: 5 do'st — VIP, 10 — moder.\n"
            "💎 <b>COINLAR</b> — Stars uchun paketlar.\n"
            "👁 <b>OCHISH</b> — VIP: oyiga 3 bepul, keyin 1⭐ yoki paket.\n"
            "🌐 <b>TIL</b> — ru/uz/en."
        ),
        "en": (
            "ℹ️ <b>HELP 𐌽𐌗ተ</b>\n"
            ...
            "• Question or valentine: text, voice.\n"
            "• Premium: photos, stickers, gifs, videos.\n"
            "• Reply, report, reveal sender, delete for both.\n"
            "• Code change: once per 3 days, Premium — unlimited.\n"
            "• Limit: 20 anon msgs/day (Premium — unlimited).\n"
            "🎲 <b>ROULETTE</b> — random partner.\n"
            "• Choose who to find, VIP — queue priority.\n"
            "🎯 <b>𝐍𝐞𝐱𝐭..</b> — profiles and matches.\n"
            "• Create profile, browse, like/dislike/write/report.\n"
            "• 👁 Reveal under the card — who it is, BEFORE liking.\n"
            "• «You were liked» notification — with counter.\n"
            "• Mutual like = MATCH: contact free, button straight\n"
            "  to DM with a ready greeting.\n"
            "👤 <b>PROFILE</b> — stats, gender/age, gift coins.\n"
            "🛒 <b>SHOP (coins)</b> — VIP 7/30 days, coins, moder app.\n"
            " <b>VIP PERKS</b>:\n"
            "• 🎁 +5 coins daily.\n"
            "• 💎 −20% in shop.\n"
            "• 👁 3 free «Reveal»/month.\n"
            "• 🚀 Priority in 𝐍𝐞𝐭.. and roulette.\n"
            "• 🔗 Unlimited link change.\n"
            "• 🚫 No ads + media in anon msgs.\n"
            "⭐ <b>PREMIUM</b> — bought ONLY in 🛒 Shop for coins!\n"
            "👥 <b>INVITE</b> — +50 coins per friend (Premium +100).\n"
            "• Rewards: VIP for 5 friends, moder for 10.\n"
            "💎 <b>COINS</b> — packages for Stars.\n"
            "👁 <b>REVEAL</b> — VIP: 3 free/mo, then 1⭐ or packs.\n"
            "🌐 <b>LANGUAGE</b> — ru/uz/en."
        ),
    },
    "profile_full": {
        "ru": (
    ...
    "age_enter_number": {
        "ru": "🔢 Введите возраст числом:",
        "uz": "🔢 Yoshni raqam bilan:",
        "en": "🔢 Enter age as number:",
    },
    "age_saved": {
        "ru": "✅ Возраст: <b>{age}</b>",
        "uz": "✅ Yosh: <b>{age}</b>",
    ...
    "giftcoins_ask_id": {
        "ru": "🎁 Введите ID или @username друга:",
        "uz": "🎁 Do'stning ID yoki @username ini kiriting:",
        "en": "🎁 Enter friend's ID or @username:",
    },
    "giftcoins_ask_amount": {
        "ru": "💰 Сколько коинов? Баланс: <b>{balance}</b>",
        "uz": "💰 Qancha coin? Balans: <b>{balance}</b>",
        "en": "💰 How many coins? Balance: <b>{balance}</b>",
    },
    ...
    "giftcoins_not_enough": {
        "ru": "💸 Недостаточно. Баланс: <b>{balance}</b>",
        "uz": "💸 Yetarli emas. Balans: <b>{balance}</b>",
        "en": "💸 Not enough. Balance: <b>{balance}</b>",
    },
    "giftcoins_sent": {
        "ru": "✅ Подарено <b>{amount}</b> → <code>{id}</code>",
        "uz": "✅ <b>{amount}</b> sovg'a → <code>{id}</code>",
        "en": "✅ Gifted <b>{amount}</b> → <code>{id}</code>",
    },
    "giftcoins_received": {
        "ru": "🎁 Вам подарили <b>{amount}</b>!",
        "uz": "🎁 Sizga <b>{amount}</b> sovg'a!",
        "en": "🎁 You received <b>{amount}</b>!",
    },
    "gift_user_not_found": {
        "ru": "❌ Не найден. Введите ID:",
        "uz": "❌ Topilmadi. ID kiriting:",
        "en": "❌ Not found. Enter ID:",
    },
    "gift_not_self": {
        "ru": "❌ Нельзя себе. Введите ID:",
        "uz": "❌ O'zingizga. ID kiriting:",
        "en": "❌ Can't self. Enter ID:",
    },
}
===================== БЛОК 5 / 16 — СЛОВАРЬ T (часть 2) =====================
T.update({
    "link_section": {
    ...
    "btn_reveal": {"ru": "👁 Узнать · 1 ⭐", "uz": "👁 Aniqlash · 1 ⭐", "en": "👁 Reveal · 1 ⭐"},
    ...
    "roulette_found": {"ru": "🎲🟢 <b>СОБЕСЕДНИК НАЙДЕН</b> 🟢🎲\n<i>Пиши первым!</i>\n<blockquote>🤫 Анонимно · фото, голосовые, стикеры\n«Далее» — другой · «Стоп» — выйти</blockquote>", "uz": "🎲🟢 <b>SUHBATDOSH TOPILDI</b> 🟢🎲\n<i>Birinchi yozing!</i>", "en": "🎲🟢 <b>PARTNER FOUND</b> 🟢🎲\n<i>Write first!</i>"},
    ...
    "btn_reveal_yes": {"ru": "✅ Раскрыть · 1 ⭐", "uz": "✅ Aniqlash · 1 ⭐", "en": "✅ Reveal · 1 ⭐"},
    ...
    # ИСПРАВЛЕНО: Убрал "⚠️ Если друг заблокирует бота — коины спишутся"
    "referral_screen": {
    ...
    "ref_rewards_title": {"ru": "🏆 <b>Награды</b>\n👑 Premium — за {vip_n} друзей ({vip_d} дн.)\n🛡 Модер — за {mod_n} ({mod_d} дн.)", "uz": "🏆 <b>Mukofotlar</b>\n👑 {vip_n} do'st ({vip_d} kun)\n🛡 {mod_n} ({mod_d} kun)", "en": "🏆 <b>Rewards</b>\n👑 {vip_n} friends ({vip_d}d)\n🛡 {mod_n} ({mod_d}d)"},
    ...
    "moder_form_sent": {"ru": "📨 Отправлено администратору", "uz": "📨 Adminga yuborildi", "en": "📨 Sent to admin"},
    ...
    # ИСПРАВЛЕНО: Убрал VIP за Stars, добавил подсказку про магазин
    "premium_title": {
        "ru": "⭐ <b>Premium</b>\n━━━━━━━━━━━━━━━━━━━━\n• 🚀 Приоритет в 𝐍𝐞𝐱𝐭..\n• 🚫 Без рекламы\n• 👁 3 «Узнать»/мес\n• 💎 −20% в магазине\n• 🎲 Приоритет в рулетке\n• 🔗 Смена ссылки без ограничений\n\n💰 <b>Покупается в 🛒 Магазине за коины!</b>\n\n👇 Выбери:",
        "uz": "⭐ <b>Premium</b>\n━━━━━━━━━━━━━━━━━━━━\n• 🚀 𝐍𝐞𝐱𝐭.. ustuvorlik\n• 🚫 Reklamasiz\n• 👁 3 «Aniqlash»/oy\n• 💎 Do'konda −20%\n• 🎲 Ruletkada ustuvorlik\n• 🔗 Havola cheklovsiz\n\n💰 <b>🛒 Do'konda coinlar evaziga!</b>\n\n👇 Tanlang:",
        "en": "⭐ <b>Premium</b>\n━━━━━━━━━━━━━━━━━━━━\n• 🚀 Priority in 𝐍𝐞𝐱𝐭..\n• 🚫 No ads\n• 👁 3 «Reveal»/month\n• 💎 −20% shop\n• 🎲 Priority roulette\n• 🔗 Unlimited link change\n\n💰 <b>Bought in 🛒 Shop for coins!</b>\n\n👇 Choose:",
    },
    ...
    # НОВЫЕ КЛЮЧИ ДЛЯ NEXT..
    "next_teaser": {
        "ru": "💕 <b>Ты понравилась {n} человеку(ам)!</b>\nКто-то лайкнул твою анкету в 𝐍𝐞𝐱𝐭..\nЛайкни в ответ — и будет мэтч 🎉",
        "uz": "💕 <b>Sizni {n} kishi yoqtirdi!</b>\nKimdir 𝐍𝐞𝐱𝐭.. da anketingizga layk qoldirdi.\nJavob layk — va match bo'ladi 🎉",
        "en": "💕 <b>{n} people liked you!</b>\nSomeone liked your profile in 𝐍𝐞𝐱𝐭..\nLike back — and it's a match 🎉",
    },
    "match_goto_hint": {
        "ru": "👇 Кнопка ниже — сразу в ЛС, приветствие уже написано:",
        "uz": "👇 Pastdagi tugma — to'g'ridan-to'g'ri DM ga, salom yozilgan:",
    ...
    "auto_hello_url": {
        "ru": "👋 Привет! Я {name}, из 𝐍𝐞𝐱𝐭.. — хочу познакомиться 😊",
        "uz": "👋 Salom! Men {name}, 𝐍𝐞𝐱𝐭.. dan — tanishmoqchiman 😊",
        "en": "👋 Hi! I'm {name}, from 𝐍𝐞𝐱𝐭.. — let's chat 😊",
    },
})
===================== БЛОК 6 / 16 — t() И КЛАВИАТУРЫ =====================
...
def main_menu_kb(tg_id):
    ...
    try:
        has_pkg = conn.execute("SELECT 1 FROM star_packages WHERE active=1 LIMIT 1").fetchone()
        if has_pkg:
            rows.append([KeyboardButton(tr_btn("💎 Купить коины"))])
    except Exception: pass
    if is_admin(tg_id):
        rows.append([KeyboardButton("🛠 Админка")])
    elif is_moder(get_user(tg_id)):
...
def share_kb(link, text):
    share_url = "https://t.me/share/url?url=" + urllib.parse.quote(link, safe="") + "&text=" + urllib.parse.quote(text, safe="")
...
def next_final_match_kb(partner_id):
    # Ссылка с готовым приветствием (fallback на tg://user если нет username)
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(tr_btn("👤 Перейти в профиль"), url=f"tg://user?id={partner_id}")]
    ])
...
# ИСПРАВЛЕНО: Убрал VIP за Stars (1/2/3 месяца), оставил только "Купить Узнать"
def premium_menu_kb():
    return tr_kb(ReplyKeyboardMarkup([
...
def reveal_pack_kb():
    ...
    for p in REVEAL_PACKAGES:
        if p["discount"] > 0:
            label = f"{p['count']} шт — {p['stars']} ⭐ (−{p['discount']}%)"
        else:
            label = f"{p['count']} шт — {p['stars']} ⭐"
...
def ref_rewards_kb(uid, link=None):
    ...
    if link:
        ...
        share_url = "https://t.me/share/url?url=" + urllib.parse.quote(full, safe="") + "&text=" + urllib.parse.quote(t("ref_share_text"), safe="")
    ...
    rows.append([InlineKeyboardButton(t("ref_claim_coins_btn", n=n_coins, v=REF_REWARD_VIP), callback_data="ref_info")])
    rows.append([InlineKeyboardButton(t("ref_claim_vip_btn"), callback_data="claim_vip")])
    rows.append([InlineKeyboardButton(t("ref_claim_moder_btn"), callback_data="claim_moder")])
...
def shop_edit_item_kb(item):
    ...
    try: rtype = item["reward_type"]
    except: rtype = "manual"
    if rtype == "coins": rows.append([KeyboardButton("💎 Сумма коинов")])
    elif rtype == "vip": rows.append([KeyboardButton("⏳ Срок VIP")])
...
def vip_bulk_menu_kb(bulk_filter):
    if bulk_filter == "all": give, take = "➕ Выдать VIP всем", "➖ Забрать у всех"
    elif bulk_filter == "female": give, take = "➕ Выдать VIP девушкам", "➖ Забрать у девушек"
    else: give, take = "➕ Выдать VIP парням", "➖ Забрать у парней"
    return tr_kb(ReplyKeyboardMarkup([[KeyboardButton(give), KeyboardButton(take)], [KeyboardButton("⬅️ Назад")]], resize_keyboard=True))
...
def adm_channels_kb(uid=None):
    rows = [[KeyboardButton("➕ Добавить канал")], [KeyboardButton("🗑 Удалить канал")]]
    if uid is not None and is_admin(uid):
...
def moder_menu_kb():
    return tr_kb(ReplyKeyboardMarkup([
        ...
        [KeyboardButton("🛡 Помощь модера")], # ИСПРАВЛЕНО: было "ℹ️ Помощь"
        [KeyboardButton("⬅️ Назад")],
    ], resize_keyboard=True))
...
def subscribe_kb(msg_id, channels):
    ...
    for c in channels:
        ...
        if is_valid_btn_url(url): rows.append([InlineKeyboardButton(channel_title(c), url=url)])
...
def get_or_create_ref_code(uid):
    ...
    for _ in range(50):
        ...
        existing = conn.execute("SELECT 1 FROM users WHERE ref_code=?", (code,)).fetchone()
        if not existing:
...
def qualified_referrals(uid):
    row = conn.execute(
        "SELECT COUNT(*) c FROM referrals r "
        "JOIN users u ON u.tg_id=r.referred_id "
        "WHERE r.referrer_id=? AND r.active=1 AND u.custom_link IS NOT NULL", (uid,)
    ).fetchone()
...
def save_link_flow(user_id, target_id, state, msg_type=None):
    try:
        ...
        conn.execute("INSERT INTO link_flow (user_id, target_id, msg_type, state, updated_at) VALUES (?, ?, ?, ?, ?)",
...
def load_link_flow(user_id):
    ...
    try:
        ...
        if now_dt() - updated > timedelta(hours=LINK_FLOW_TTL_HOURS):
...
async def build_start_link(context, code):
    ...
    return f"t.me/{uname}?start={code}"
...
def can_show_profile(viewer_id, target_id):
    try:
        ...
        row = conn.execute("SELECT COUNT(*) c FROM nearby_shown WHERE viewer_id=? AND target_id=? AND shown_at>?", (viewer_id, target_id, week_ago)).fetchone()
...
def mark_shown(viewer_id, target_id):
    try:
        conn.execute("INSERT INTO nearby_shown (viewer_id, target_id, shown_at) VALUES (?, ?, ?)", (viewer_id, target_id, now_iso()))
        conn.commit()
    except Exception as e: log.debug("mark_shown: %s", e)

def total_profiles_count():
    try:
        row = conn.execute("SELECT COUNT(*) c FROM nearby_profiles WHERE active=1").fetchone()
        return row["c"] if row else 0
    except: return 0
...
def get_reveal_free_left(uid):
    ...
    try:
        row = conn.execute("SELECT used FROM reveal_free_used WHERE user_id=? AND period=?", (uid, period)).fetchone()
...
def use_reveal_free(uid):
    period = now_dt().strftime("%Y-%m")
    try:
        row = conn.execute("SELECT * FROM reveal_free_used WHERE user_id=? AND period=?", (uid, period)).fetchone()
        if row: conn.execute("UPDATE reveal_free_used SET used = used + 1 WHERE id=?", (row["id"],))
        else: conn.execute("INSERT INTO reveal_free_used (user_id, period, used) VALUES (?, ?, 1)", (uid, period))
        conn.commit()
...
def register_notification(uid, chat_id, message_id, ntype, target_id=None, like_id=None):
    try:
        ...
        conn.execute("INSERT INTO nearby_notifications (user_id, chat_id, message_id, type, target_id, like_id, expires_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
...
# ИСПРАВЛЕНО: В списке команд Telegram оставлен ТОЛЬКО /start
...
===================== БЛОК 7 / 16 — НАВИГАЦИЯ, УТИЛИТЫ, ГЛОБ. ОТМЕНА =====================
...
async def send_menu(bot, chat_id, user_data, text,
...
async def nav(bot, chat_id, user_data, message, text,
    ...
    return await send_menu(bot, chat_id, user_data, text,
...
async def notify_staff(context, text, reply_markup=None, parse_mode=None):
    ...
    try:
        rows = conn.execute(
            "SELECT tg_id FROM users WHERE is_moder=1 "
            "OR (moder_until IS NOT NULL AND moder_until>?)",
            (now_iso(),)).fetchall()
        for r in rows:
            targets.add(r["tg_id"])
    except Exception as e:
...
async def grant_daily_bonus(uid, context):
    ...
    try:
        ...
        await context.bot.send_message(
            uid, t("vip_daily_bonus", n=VIP_DAILY_BONUS), parse_mode="HTML")
        set_cur_lang(_sl)
    except TelegramError as e:
...
async def deliver_start_menu(context, uid, greet=True):
    ...
    try:
        if not user["gender"]:
            ...
            await context.bot.send_message(
                uid, t("welcome", name=name), parse_mode="HTML",
                reply_markup=gender_kb())
        elif user_age_int(user) is None:
            UD[uid]["state"] = "set_age_first"
            await context.bot.send_message(
                uid, t("age_register_ask"), parse_mode="HTML",
                reply_markup=ReplyKeyboardRemove())
        elif greet:
            UD[uid]["state"] = None
            await context.bot.send_message(
                uid, t("welcome_back", name=name), parse_mode="HTML",
                reply_markup=main_menu_kb(uid))
        else:
            UD[uid]["state"] = None
            await context.bot.send_message(
                uid, t("main_menu"), reply_markup=main_menu_kb(uid))
    finally:
        set_cur_lang(_sl)
...
def is_banned_pair(u1, u2):
    try:
        row = conn.execute(
            ...
            (now_iso(), u1, u2, u2, u1)).fetchone()
        return row is not None
    except Exception:
        return False
LINK_ALPHABET = string.ascii_letters + string.digits + "_-"
def valid_link_code(code):
    if not code or len(code) < 1 or len(code) > 10:
        return False
    for c in code:
        if c not in LINK_ALPHABET:
            return False
    return True
...
def user_is_disposable(uid):
    ...
    try:
        ...
        if conn.execute("SELECT 1 FROM nearby_profiles WHERE user_id=? LIMIT 1",
                        (uid,)).fetchone():
            return False
        if conn.execute("SELECT 1 FROM nearby_matches "
...
def purge_user(uid, force=False):
    try:
        ...
        for r in partner_rows:
            try:
                if r["user1_id"] == uid:
                    partner_ids.add(r["user2_id"])
                else:
                    partner_ids.add(r["user1_id"])
            except (KeyError, IndexError):
                pass
        ...
        conn.execute(
            "UPDATE roulette_sessions SET active=0, "
        ...
        conn.execute("DELETE FROM nearby_reveals WHERE viewer_id=? OR target_id=?",
                     (uid, uid))
        conn.execute("DELETE FROM reveal_free_used WHERE user_id=?", (uid,))
        conn.execute("DELETE FROM reveal_paid WHERE user_id=?", (uid,))
...
# ---------- НОВОЕ: баланс платных «Раскрыть» (паки за Stars) ----------
def get_reveal_paid_left(uid):
    try:
        row = conn.execute(
            "SELECT total, used FROM reveal_paid WHERE user_id=?",
            (uid,)).fetchone()
        if not row:
            return 0
        return max(0, (row["total"] or 0) - (row["used"] or 0))
    except Exception:
        return 0
def add_reveal_paid(uid, n):
    try:
        row = conn.execute(
            "SELECT user_id FROM reveal_paid WHERE user_id=?", (uid,)).fetchone()
        if row:
            conn.execute(
                "UPDATE reveal_paid SET total = total + ? WHERE user_id=?",
                (n, uid))
        else:
            conn.execute(
                "INSERT INTO reveal_paid (user_id, total, used) VALUES (?, ?, 0)",
                (uid, n))
        conn.commit()
    except Exception as e:
        log.warning("add_reveal_paid: %s", e)
def use_reveal_paid(uid):
    try:
        conn.execute(
            "UPDATE reveal_paid SET used = used + 1 "
            "WHERE user_id=? AND total > used", (uid,))
        conn.commit()
        return True
    except Exception as e:
        log.warning("use_reveal_paid: %s", e)
        return False
def is_revealed_pair(viewer, target):
    try:
        row = conn.execute(
            "SELECT 1 FROM nearby_reveals WHERE viewer_id=? AND target_id=?",
            (viewer, target)).fetchone()
        return row is not None
    except Exception:
        return False
def mark_revealed(viewer, target):
    try:
        conn.execute(
            "INSERT OR IGNORE INTO nearby_reveals "
            "(viewer_id, target_id, created_at) VALUES (?, ?, ?)",
            (viewer, target, now_iso()))
        conn.commit()
    except Exception as e:
        log.debug("mark_revealed: %s", e)
def reveal_btn_label(uid):
    u = get_user(uid)
    if u and is_vip(u):
        free = get_reveal_free_left(uid)
        if free > 0:
            return t("reveal_next_btn_free", n=free)
    left = get_reveal_paid_left(uid)
    if left > 0:
        return t("reveal_left_btn", n=left)
    return t("reveal_next_btn_paid", n=1)
# ---------- НОВОЕ: дневной лимит анонимок (20/сутки, VIP без лимита) ----------
def anon_count_today(uid):
    day = now_dt().strftime("%Y-%m-%d")
    try:
        row = conn.execute(
            "SELECT COUNT(*) c FROM anon_messages "
            "WHERE from_id=? AND created_at>=?", (uid, day)).fetchone()
        return row["c"] if row else 0
    except Exception:
        return 0
def anon_limit_ok(user_row, uid):
    if is_unlimited(user_row) or is_vip(user_row):
        return True, None
    if anon_count_today(uid) >= DAILY_LIMIT:
        return False, t("anon_limit", n=DAILY_LIMIT)
    return True, None
# ---------- ГЛОБАЛЬНАЯ ОТМЕНА (раньше всех стейт-хендлеров) ----------
CANCEL_TEXTS = {"❌ Отмена", "❌ Bekor qilish", "❌ Cancel"}
@dp.message(F.text.in_(CANCEL_TEXTS))
async def global_cancel(message: Message):
    uid = message.from_user.id
    st = UD[uid]
    st["state"] = None
    for key in ("anon_target", "anon_msg_type", "next_draft", "gift",
                "pkg", "adm_target", "bcast_filter", "bulk_filter",
                "next_current", "report_target"):
        st.pop(key, None)
    clear_link_flow(uid)
    await _reply(message, t("cancelled"), reply_markup=main_menu_kb(uid))
===================== БЛОК 8 / 16 — ХЕНДЛЕРЫ Ч.1 =====================
...
async def _welcome_flow(bot, uid, greet=True):
    ...
    if not u["gender"]:
        ...
        UD[uid]["state"] = "set_age_first"
        await _say(bot, uid, t("age_register_ask"),
                   reply_markup=ReplyKeyboardRemove())
    elif greet:
        UD[uid]["state"] = None
        await _say(bot, uid, t("welcome_back", name=name),
                   reply_markup=main_menu_kb(uid))
    else:
        UD[uid]["state"] = None
...
async def cmd_start(message: Message, command: CommandObject):
    ...
    if not await _gate_ok(message.bot, uid):
        chans = await get_mandatory_channels()
        await _reply(message, t("subgate_start"),
        ...
        if ref_id and ref_id != uid and get_user(ref_id):
            try:
                conn.execute(
                    ...
                    "(referrer_id, referred_id, coins_awarded, active, created_at) "
                    "VALUES (?, ?, 0, 1, ?)", (ref_id, uid, now_iso()))
                conn.commit()
                ref_user = get_user(ref_id)
                bonus = REF_INVITED_BONUS_VIP if is_vip(ref_user) \
                    else REF_INVITED_BONUS
...
async def cmd_help(message: Message):
    ...
    ensure_user(uid, message.from_user.username, message.from_user.first_name)
    UD[uid]["state"] = None
    await _reply(message, t("help"), reply_markup=main_menu_kb(uid))
...
async def _render_profile(uid):
    ...
    try:
        rows = conn.execute(
            ...
            "WHERE user1_id=? OR user2_id=?", (uid, uid)).fetchall()
        for r in rows:
            try:
                start = datetime.fromisoformat(r["started_at"])
                end_str = r["ended_at"]
                end = now_dt() if (r["active"] or not end_str) \
                    else datetime.fromisoformat(end_str)
    ...
    try:
        row = conn.execute(
            "SELECT COUNT(*) c FROM referrals WHERE referrer_id=? AND active=1",
            (uid,)).fetchone()
        if row:
            invited = row["c"]
    except Exception:
    ...
    try:
        rows = conn.execute(
            ...
            "GROUP BY referrer_id ORDER BY c DESC LIMIT 100").fetchall()
    ...
    try:
        row = conn.execute(
            "SELECT COALESCE(SUM(stars),0) s FROM star_purchases "
            "WHERE user_id=? AND refunded=0", (uid,)).fetchone()
        if row:
            stars = row["s"]
    except Exception:
        pass
    ...
    try:
        row = conn.execute(
            "SELECT COUNT(*) c FROM anon_messages WHERE from_id=?",
            (uid,)).fetchone()
        if row:
            sent = row["c"]
    except Exception:
    ...
    try:
        row = conn.execute(
            "SELECT COUNT(*) c FROM anon_messages WHERE to_id=?",
            (uid,)).fetchone()
        if row:
            received = row["c"]
    except Exception:
        pass
    return t("profile_full", id=uid,
             name=html.escape(u["first_name"] or "—"),
             gender=gender_label(u["gender"]), age=u["age"] or "—",
             roulette_time=fmt_duration(roulette_secs), sent=sent,
             received=received, invited=invited, rank=rank, vip=vip_str,
             coins=u["coins"] or 0, stars=stars, reg_date=reg_date)
@dp.message(F.text.in_({"👤 Профиль", "👤 Profil", "👤 Profile"}))
async def btn_profile(message: Message):
    uid = message.from_user.id
    ensure_user(uid, message.from_user.username, message.from_user.first_name)
    UD[uid]["state"] = None
    await _reply(message, await _render_profile(uid), reply_markup=profile_kb())
@dp.message(F.text.in_({"✏️ Сменить пол", "✏️ Jinsni o'zgartirish", "✏️ Change gender"}))
...
@dp.message(F.text.in_({"✏️ Изменить возраст", "✏️ Yoshni o'zgartirish", "✏️ Change age"}))
...
@dp.message(F.text.in_({"🎁 Подарить коины", "🎁 Coin sovg'a qilish", "🎁 Gift coins"}))
...
@dp.message(F.text.in_({"🌐 Язык", "🌐 Til", "🌐 Language"}))
...
@dp.message(F.text.in_({"ℹ️ Помощь", "ℹ️ Yordam", "ℹ️ Help"}))
async def btn_help(message: Message):
    await _reply(message, t("help"),
                 reply_markup=main_menu_kb(message.from_user.id))
@dp.message(F.text.in_({"🛡 Помощь модера", "🛡 Moderator yordami", "🛡 Moder help"}))
async def btn_moder_help(message: Message):
    uid = message.from_user.id
    if not is_staff(uid):
        return
    await _reply(message, t("moder_help"), reply_markup=moder_menu_kb())
...
async def _render_referral(uid):
    ...
    try:
        row = conn.execute(
            "SELECT COALESCE(SUM(coins_awarded),0) s FROM referrals "
            "WHERE referrer_id=?", (uid,)).fetchone()
        if row:
            earned = row["s"]
    except Exception:
        pass
    bonus = t("referral_bonus_vip") if (u and is_vip(u)) \
        else t("referral_bonus_normal")
    return t("referral_screen", reward=reward, bonus=bonus,
             total=total, earned=earned, link=link)
@dp.message(F.text.in_({"👥 Пригласить", "👥 Taklif qilish", "👥 Invite"}))
async def btn_referral(message: Message):
    ...
    try:
        ...
        pass
@dp.message(F.text.in_({"🏆 Топ пригласивших", "🏆 Top taklif qilganlar",
                        "🏆 Top inviters"}))
async def btn_top_ref(message: Message):
    ...
    await _reply(message, "\n".join(lines),
                 reply_markup=referral_kb(message.from_user.id))
@dp.message(F.text.in_({"✏️ Изменить", "✏️ O'zgartirish", "✏️ Edit"}))
async def btn_ref_settings(message: Message):
    uid = message.from_user.id
    if not is_admin(uid):
        return
    UD[uid]["state"] = None
    await _reply(message, t("choose_action"), reply_markup=ref_settings_kb())
@dp.message(F.text.in_({"👑 VIP: дней", "👑 VIP: kun", "👑 VIP: days"}))
async def rs_vip_days(message: Message):
    if not is_admin(message.from_user.id):
        return
    UD[message.from_user.id]["state"] = "ref_set_vip_days"
    await _reply(message, t("enter_number"), reply_markup=cancel_reply_kb())
@dp.message(F.text.in_({"👥 VIP: друзей", "👥 VIP: do'st", "👥 VIP: friends"}))
async def rs_vip_friends(message: Message):
    if not is_admin(message.from_user.id):
        return
    UD[message.from_user.id]["state"] = "ref_set_vip_friends"
    await _reply(message, t("enter_number"), reply_markup=cancel_reply_kb())
@dp.message(F.text.in_({"🛡 Модер: дней", "🛡 Moder: kun", "🛡 Moder: days"}))
async def rs_moder_days(message: Message):
    if not is_admin(message.from_user.id):
        return
    UD[message.from_user.id]["state"] = "ref_set_moder_days"
    await _reply(message, t("enter_number"), reply_markup=cancel_reply_kb())
@dp.message(F.text.in_({"👥 Модер: друзей", "👥 Moder: do'st", "👥 Moder: friends"}))
async def rs_moder_friends(message: Message):
    if not is_admin(message.from_user.id):
        return
    UD[message.from_user.id]["state"] = "ref_set_moder_friends"
    await _reply(message, t("enter_number"), reply_markup=cancel_reply_kb())
_REF_SET_KEYS = {
    "ref_set_vip_days": "ref_vip_days",
    "ref_set_vip_friends": "ref_vip_threshold",
    "ref_set_moder_days": "ref_moder_days",
    "ref_set_moder_friends": "ref_moder_threshold",
}
@dp.message(F.text, lambda m: (
    UD[m.from_user.id].get("state") or "").startswith("ref_set_"))
async def ref_settings_save(message: Message):
    uid = message.from_user.id
    if not is_admin(uid):
        return
    ...
    if not text.isdigit() or int(text) <= 0:
        await _reply(message, t("enter_number"), reply_markup=cancel_reply_kb())
        return
    key = _REF_SET_KEYS.get(UD[uid]["state"])
    if key:
        set_setting(key, int(text))
    UD[uid]["state"] = None
    await _reply(message, t("done"), reply_markup=ref_settings_kb())
===================== БЛОК 9 / 16 — ССЫЛКА И АНОНИМКИ =====================
def _link_url(uid):
    u = get_user(uid)
    if not u:
        return ""
    ...
    if not clink:
        return ""
    bot_uname = _BOT_USERNAME or "bot"
    return f"https://t.me/{bot_uname}?start=link_{clink}"
@dp.message(F.text.in_({"🔗 Моя ссылка", "🔗 Havolam", "🔗 My link"}))
...
@dp.message(F.text.in_({"🔗 Показать ссылку", "🔗 Havolani ko'rsatish", "🔗 Show link"}))
async def btn_show_link(message: Message):
    ...
    try:
        ...
        pass
@dp.message(F.text.in_({"✏️ Сменить ссылку", "✏️ Havolani o'zgartirish", "✏️ Change link"}))
...
@dp.message(
    F.text.regexp(r"^[A-Za-z0-9_-]{1,10}$"),
    lambda m: UD[m.from_user.id].get("state") in ("link_create", "link_change"),
)
async def handle_link_code(message: Message):
    ...
    if not valid_link_code(code):
        ...
        return
    exists = conn.execute(
        "SELECT 1 FROM users WHERE custom_link=? AND tg_id<>?",
        (code, uid)).fetchone()
    if exists:
...
async def handle_anon_deeplink(message, code):
    ...
    ensure_user(uid, message.from_user.username, message.from_user.first_name)
    target = conn.execute(
        "SELECT * FROM users WHERE custom_link=? OR old_link=?",
        (code, code)).fetchone()
    if not target:
...
async def _send_anon_to_target(bot, from_id, target_id, msg_type, content_type,
    ...
    try:
        if content_type == "text":
            ...
            sent = await bot.send_animation(target_id, animation_file_id,
                                            caption=recv_hdr)
        ...
        return None
    owner_mid = sent.message_id if sent else 0
    mid = None
    try:
        cur = conn.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?, 0, 0, ?)",
            (from_id, target_id, msg_type, content_type, text,
             voice_file_id, owner_mid, now_iso()))
        conn.commit()
        mid = cur.lastrowid
    except Exception as e:
...
async def _fanout_anon_watchers(bot, from_id, to_id, text, content_type):
    try:
        watchers = conn.execute(
            "SELECT DISTINCT mod_id FROM anon_watchers "
            "WHERE target_id=? AND active=1", (to_id,)).fetchall()
    except Exception:
        return
    if not watchers:
        return
    sfu = get_user(from_id)
    sname = sfu["first_name"] if sfu else "—"
    ...
    for w in watchers:
        ...
        try:
            ...
            head = t("anon_watch_new_msg", uid=to_id,
                     sname=html.escape(sname), sfid=from_id, when=when)
            set_cur_lang(_sl)
...
async def handle_voice_msg(message: Message):
    ...
    if state == "anon_text":
        ...
        if not target_id:
            return
        u = get_user(uid)
        ok, err = anon_limit_ok(u, uid)
        if not ok:
            UD[uid]["state"] = None
            UD[uid].pop("anon_target", None)
            await _reply(message, err, reply_markup=main_menu_kb(uid))
            return
        ...
        UD[uid].pop("anon_target", None)
        if mid:
            await _reply(message, t("anon_sent"), reply_markup=main_menu_kb(uid))
        else:
            await _reply(message, t("anon_failed"), reply_markup=main_menu_kb(uid))
        return
    ...
    if _sex_user_room(uid):
        ...
        return
    if state and state.startswith("next_msg:"):
        try:
            partner = int(state.split(":", 1)[1])
        except (ValueError, IndexError):
            return
        await _next_relay(message, uid, partner,
                          voice_file_id=message.voice.file_id)
        return
    if state and state.startswith("next_reply_to:"):
        try:
            partner = int(state.split(":", 1)[1])
        except (ValueError, IndexError):
            return
        await _next_relay(message, uid, partner,
                          voice_file_id=message.voice.file_id)
        return
# ---------- НОВОЕ: VIP-медиа в анонимках (фото/стикер/гиф/видео) ----------
@dp.message(
    F.photo | F.sticker | F.animation | F.video,
    lambda m: UD[m.from_user.id].get("state") == "anon_text",
)
async def handle_anon_media(message: Message):
    uid = message.from_user.id
    u = get_user(uid)
    target_id = UD[uid].get("anon_target")
    msg_type = UD[uid].get("anon_msg_type", "question")
    if not target_id:
        ...
        return
    if not (is_vip(u) or is_unlimited(u)):
        await _reply(message, t("anon_vip_media"), reply_markup=cancel_reply_kb())
        return
    ok, err = anon_limit_ok(u, uid)
    if not ok:
        UD[uid]["state"] = None
        ...
        return
    if message.photo:
        ct, fid = "photo", message.photo[-1].file_id
    elif message.sticker:
        ct, fid = "sticker", message.sticker.file_id
    elif message.animation:
        ct, fid = "animation", message.animation.file_id
    else:
        ct, fid = "video", message.video.file_id
    mid = await _send_anon_to_target(
        message.bot, uid, target_id, msg_type, ct, None,
        **{ct + "_file_id": fid})
    UD[uid]["state"] = None
    UD[uid].pop("anon_target", None)
    if mid:
...
async def _do_anon_reply(message, uid, parent_id, voice_file_id=None):
    parent = conn.execute(
        "SELECT * FROM anon_messages WHERE id=?", (parent_id,)).fetchone()
    ...
    try:
        conn.execute(
            "INSERT INTO anon_messages "
            "(from_id, to_id, msg_type, content_type, text, voice_file_id, "
            "parent_id, owner_chat_message_id, answered, deleted, created_at) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, 0, ?)",
            (uid, target_id, parent["msg_type"],
             "voice" if voice_file_id else "text", reply_text, voice_file_id,
             parent_id, sent.message_id, now_iso()))
        conn.commit()
    except Exception as e:
        log.warning("anon reply DB: %s", e)
    UD[uid]["state"] = None
    await _reply(message, t("anon_reply_sent"), reply_markup=main_menu_kb(uid))
async def handle_text_router(message):
    uid = message.from_user.id
    text = (message.text or "").strip()
    u = get_user(uid)
    if not u:
    ...
    if state == "gift_amount":
        ...
        amt = int(text)
        u2 = get_user(uid)
        if amt > (u2["coins"] or 0):
            await _reply(message, t("giftcoins_not_enough", balance=u2["coins"] or 0),
                         reply_markup=cancel_reply_kb())
            return
    ...
    if state == "anon_text":
        ...
        if has_forbidden_contacts(text) and not is_vip(u):
            ...
            return
        mid = await _send_anon_to_target(message.bot, uid, target_id,
                                         msg_type, "text", text)
        UD[uid]["state"] = None
        UD[uid].pop("anon_target", None)
        if mid:
...
async def _relay_roulette(message, session):
    uid = message.from_user.id
    other = session["user2_id"] if session["user1_id"] == uid \
        else session["user1_id"]
    if message.text and has_forbidden_contacts(message.text):
        if not is_staff(uid):
...
===================== БЛОК 10 / 16 — РУЛЕТКА =====================
@dp.message(F.text.in_({"🎲 Чат-рулетка", "🎲 Chat-ruletka", "🎲 Chat roulette"}))
async def btn_roulette(message: Message):
    uid = message.from_user.id
    u = get_user(uid)
    if not u or not u["gender"]:
        UD[uid]["state"] = "set_gender_first"
        await _reply(message, t("gender_needed_for_search"),
                     reply_markup=gender_kb())
        return
    if get_active_session(uid):
        await _reply(message, t("roulette_already_chat"),
                     reply_markup=in_chat_kb())
        return
    UD[uid]["state"] = "roulette_pref"
    await _reply(message, t("roulette_who"),
...
async def _roulette_join(message, pref):
    ...
    try:
        conn.execute("DELETE FROM roulette_queue WHERE user_id=?", (uid,))
        is_v = 1 if is_vip(u) else 0
        conn.execute(
            "INSERT INTO roulette_queue "
            "(user_id, gender, pref, is_vip, mode, joined_at) "
            "VALUES (?, ?, ?, ?, 'normal', ?)",
            (uid, u["gender"], pref, is_v, now_iso()))
        conn.commit()
    except Exception as e:
...
async def _try_match():
    try:
        q = conn.execute(
            ...
            "ORDER BY is_vip DESC, joined_at ASC").fetchall()
    except Exception as e:
...
async def _start_session(u1, u2, mode="normal"):
    try:
        conn.execute(
            "DELETE FROM roulette_queue WHERE user_id IN (?, ?)", (u1, u2))
        conn.execute(
            "INSERT INTO roulette_sessions "
            "(user1_id, user2_id, active, mode, started_at) "
            "VALUES (?, ?, 1, ?, ?)", (u1, u2, mode, now_iso()))
        conn.commit()
    except Exception as e:
        log.warning("start_session: %s", e)
    for pair in (u1, u2):
        UD[pair]["state"] = None
        UD[pair]["last_partner"] = u2 if pair == u1 else u1
        try:
            _sl = cur_lang()
...
@dp.message(F.text.in_({"⛔ Отменить поиск", "⛔ Qidiruvni bekor qilish", "⛔ Stop search"}))
...
@dp.message(F.text.in_({"⏹️ Стоп", "⏹️ To'xtatish", "⏹️ Stop"}))
async def btn_stop_session(message: Message):
    ...
    conn.commit()
    UD[uid]["state"] = None
    UD[uid]["last_partner"] = other
    UD[other]["last_partner"] = uid
    ...
    try:
        ...
        pass
@dp.message(F.text.in_({"➡️ Далее", "➡️ Keyingi", "➡️ Next"}))
async def btn_next_partner(message: Message):
    ...
    try:
        ...
        log.warning("try_match: %s", e)
@dp.message(F.text.in_({"🔍 Новый поиск", "🔍 Yangi qidiruv", "🔍 New search"}))
...
@dp.message(F.text.in_({"🚩 Пожаловаться", "🚩 Shikoyat qilish", "🚩 Report"}))
async def btn_report_last(message: Message):
    UD[message.from_user.id]["state"] = None
...
async def btn_report_reason(message: Message):
    ...
    reason = message.text
    UD[uid]["state"] = None
    target = UD[uid].get("last_partner")
    if target:
        rid = 0
        try:
            cur = conn.execute(
                "INSERT INTO reports "
                "(reporter_id, reported_id, context, reason, status, created_at) "
                "VALUES (?, ?, 'roulette', ?, 'pending', ?)",
                (uid, target, reason, now_iso()))
            conn.commit()
            rid = cur.lastrowid or 0
        except Exception as e:
            log.warning("roulette report insert: %s", e)
        UD[uid].pop("last_partner", None)
...
async def relay_to_spectators(bot, session, sender_id, chat_id, mid):
    ...
    for sid in list(specs):
        try:
            ...
            pass
===================== БЛОК 11 / 16 — 𝐍𝐞𝐱𝐭.. =====================
def next_reaction_kb(uid=None):
    label = reveal_btn_label(uid) if uid else t("reveal_next_btn_paid", n=1)
    return tr_kb(ReplyKeyboardMarkup([
        [KeyboardButton("❤️ Лайк"), KeyboardButton("👎 Дизлайк")],
        [KeyboardButton("💌 Написать"), KeyboardButton("🚩 Жалоба")],
...
def _greeting_plain(greeter_name):
    return t("auto_hello_url", name=greeter_name)
def contact_url(partner_row, greeter_name):
    if not partner_row:
        return ""
    if partner_row["username"]:
        greet = _greeting_plain(greeter_name)
        return ("https://t.me/" + partner_row["username"]
                + "?text=" + urllib.parse.quote(greet))
    return f"tg://user?id={partner_row['tg_id']}"
def match_goto_kb(partner_row, greeter_name):
    url = contact_url(partner_row, greeter_name)
    if not url:
        return None
    return InlineKeyboardMarkup([[
        InlineKeyboardButton(tr_btn("👤 Перейти в профиль"), url=url)]])
@dp.message(F.text.in_({"🎯 𝐞𝐱𝐭..", "🎯 𝐍𝐞𝐱𝐭..", "🎯 𝐍𝐞𝐱𝐭.."}))
async def btn_next_main(message: Message):
    uid = message.from_user.id
    prof = conn.execute(
        "SELECT * FROM nearby_profiles WHERE user_id=?", (uid,)).fetchone()
    UD[uid]["state"] = None
    if not prof:
...
@dp.message(F.text.in_({"🔥 Смотреть анкеты", "🔥 Anketalarni ko'rish", "🔥 Browse profiles"}))
...
async def btn_my_next_profile(message: Message):
    ...
    if not prof:
        ...
        await _reply(message, t("next_ask_name"), reply_markup=next_cancel_kb())
        return
    txt = t("next_preview", name=html.escape(prof["name"] or "—"),
            age=prof["age"] or "—", gender=gender_label(prof["gender"]),
...
async def _show_next_profile(message, uid):
    ...
    try:
        week_ago = (now_dt() - timedelta(days=NEXT_REPEAT_WINDOW_DAYS)).isoformat()
        if pref == "any":
            where_gender, params_gender = "", ()
        else:
            where_gender, params_gender = "AND p.gender = ?", (pref,)
        sql = ("SELECT p.* FROM nearby_profiles p "
               "WHERE p.active=1 AND p.user_id<>? " + where_gender + " "
    ...
    if not rows:
        try:
            rows = conn.execute(
                ...
                "WHERE p.active=1 AND p.user_id<>? LIMIT 500", (uid,)).fetchall()
        except Exception:
            rows = []
    if not rows:
        await _reply(message, t("next_no_profiles"), reply_markup=next_menu_kb())
    ...
    txt = (t("next_view_header", name=html.escape(prof["name"] or "—"),
             age=prof["age"] or "—")
    ...
    kb = next_reaction_kb(uid)
    try:
        if prof["photo_id"]:
            sent = await message.bot.send_photo(uid, prof["photo_id"],
                                                caption=txt, reply_markup=kb)
        else:
            sent = await message.bot.send_message(uid, txt, reply_markup=kb)
...
async def next_like(message: Message):
    ...
    try:
        ...
        conn.execute(
            ...
            "VALUES (?, ?, 'like', ?)", (uid, target, now_iso()))
        conn.commit()
    except Exception as e:
        log.warning("next like: %s", e)
    rev = conn.execute(
        "SELECT * FROM nearby_likes "
        "WHERE from_id=? AND to_id=? AND action='like'",
        (target, uid)).fetchone()
    if rev:
        await _handle_mutual_like(message.bot, uid, target)
    else:
        await _notify_like_teaser(message.bot, uid, target)
    await _show_next_profile(message, uid)
...
async def next_dislike(message: Message):
    ...
    try:
        ...
        conn.execute(
            "INSERT INTO nearby_likes (from_id, to_id, action, created_at) "
            "VALUES (?, ?, 'dislike', ?)", (uid, target, now_iso()))
        conn.commit()
    except Exception as e:
...
async def next_report(message: Message):
    ...
    try:
        cur = conn.execute(
            "INSERT INTO reports "
            "(reporter_id, reported_id, context, reason, status, created_at) "
            "VALUES (?, ?, 'next', NULL, 'pending', ?)",
            (uid, target, now_iso()))
        conn.commit()
        rid = cur.lastrowid or 0
    except Exception as e:
        log.warning("next report insert: %s", e)
    await _reply(message, t("next_report_confirm"),
                 reply_markup=next_reaction_kb(uid))
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton(t("🔨 Бан"), callback_data=f"rep:ban:{rid}"),
        InlineKeyboardButton(t("❌ Отклонить"), callback_data=f"rep:no:{rid}"),
    ]])
...
# ---------- ТИЗЕР СО СЧЁТЧИКОМ (вместо полной карточки) ----------
async def _notify_like_teaser(bot, from_id, to_id):
    try:
        week_ago = (now_dt() - timedelta(days=7)).isoformat()
        row = conn.execute(
            "SELECT COUNT(*) c FROM nearby_likes "
            "WHERE to_id=? AND action='like' AND created_at>=?",
            (to_id, week_ago)).fetchone()
        n = row["c"] if row else 1
    except Exception:
        n = 1
    try:
        _sl = cur_lang()
        set_cur_lang(get_lang(to_id))
        txt = t("next_teaser", n=n)
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton(tr_btn("🔥 Смотреть анкеты"),
                                  callback_data=f"ntease_show:{from_id}")],
            [InlineKeyboardButton(tr_btn("💤 Игнорировать"),
                                  callback_data="ntease_no")],
        ])
        set_cur_lang(_sl)
        msg = await bot.send_message(to_id, txt, reply_markup=kb)
        register_notification(to_id, to_id, msg.message_id, "like", from_id, None)
    except TelegramError as e:
        log.debug("notify_like_teaser: %s", e)
    except Exception as e:
        log.warning("teaser err: %s", e)
@dp.callback_query(F.data.startswith("ntease_show:"))
async def cb_ntease_show(cq: CallbackQuery):
    uid = cq.from_user.id
    try:
        ...
        return
    prof = conn.execute(
        "SELECT * FROM nearby_profiles WHERE user_id=? AND active=1",
        (target,)).fetchone()
    if not prof:
        await cq.answer(t("next_no_profiles"), show_alert=True)
        return
    mark_shown(uid, target)
    UD[uid]["next_current"] = target
    txt = (t("next_view_header", name=html.escape(prof["name"] or "—"),
             age=prof["age"] or "—")
           + f"\n🎯 {pref_label(prof['looking_for'])}"
           + f"\n\n📄 {html.escape(prof['bio'] or '—')}")
    old_card = UD[uid].pop("next_card_id", None)
    if old_card:
        await _safe_delete_message(cq.bot, uid, old_card)
    kb = next_reaction_kb(uid)
    try:
        if prof["photo_id"]:
            sent = await cq.bot.send_photo(uid, prof["photo_id"],
                                           caption=txt, reply_markup=kb)
        else:
            sent = await cq.bot.send_message(uid, txt, reply_markup=kb)
        UD[uid]["next_card_id"] = sent.message_id
    except TelegramError as e:
        log.warning("teaser show card: %s", e)
    await cq.answer()
@dp.callback_query(F.data == "ntease_no")
async def cb_ntease_no(cq: CallbackQuery):
    try:
        await cq.message.edit_reply_markup(reply_markup=None)
    except TelegramError:
        pass
    await cq.answer(t("liked_ignored"))
# ---------- РАСКРЫТИЕ ДО ЛАЙКА (VIP 3 бесп. → паки → 1⭐) ----------
@dp.message(
    F.text.regexp(r"^👁 (Узнать|Раскрыть|Aniqlash|Reveal|·)"),
    lambda m: UD[m.from_user.id].get("state") is None
    and UD[m.from_user.id].get("next_current"),
)
async def next_reveal_press(message: Message):
    uid = message.from_user.id
    target = UD[uid].get("next_current")
    if not target:
        return
    u = get_user(uid)
    if is_revealed_pair(uid, target):
        await _send_reveal_info(message.bot, uid, target)
        return
    if u and is_vip(u) and get_reveal_free_left(uid) > 0:
        use_reveal_free(uid)
        await _do_reveal_next(message.bot, uid, target)
        return
    if get_reveal_paid_left(uid) > 0:
        use_reveal_paid(uid)
        await _do_reveal_next(message.bot, uid, target)
        return
    try:
        await message.bot.send_invoice(
            ...
            prices=[_AiLP(label="👁 Reveal", amount=1)])
        await _reply(message, t("reveal_paying"),
                     reply_markup=next_reaction_kb(uid))
    except TelegramError as e:
        log.warning("reveal invoice: %s", e)
async def _do_reveal_next(bot, uid, target):
    mark_revealed(uid, target)
    await _send_reveal_info(bot, uid, target)
async def _send_reveal_info(bot, uid, target):
    prof = conn.execute(
        "SELECT * FROM nearby_profiles WHERE user_id=?", (target,)).fetchone()
    tu = get_user(target)
    if not prof:
        return
    ...
    _sl = cur_lang()
    set_cur_lang(get_lang(uid))
    txt = t("reveal_result_next", name=html.escape(prof["name"] or "—"),
            age=prof["age"] or "—", bio=html.escape(prof["bio"] or "—"),
            tid=target, uname_or_link=uname)
    url = ("https://t.me/" + tu["username"]) if (tu and tu["username"]) \
        else f"tg://user?id={target}"
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton(tr_btn("👤 Перейти в профиль"), url=url)]])
    set_cur_lang(_sl)
    try:
...
# ---------- МЭТЧ: бесплатно + ОДНА кнопка в ЛС с приветствием ----------
async def _handle_mutual_like(bot, u1, u2):
    ...
    for a, b in ((u1, u2), (u2, u1)):
        try:
            pb = conn.execute(
                "SELECT * FROM nearby_profiles WHERE user_id=?", (b,)).fetchone()
            pa = conn.execute(
                "SELECT * FROM nearby_profiles WHERE user_id=?", (a,)).fetchone()
            ua = get_user(a)
            ub = get_user(b)
            if not pb:
                continue
            greeter = (pa["name"] if pa else None) or \
                      (ua["first_name"] if ua else "—")
            _sl = cur_lang()
            set_cur_lang(get_lang(a))
            info = t("match_final_info", name=html.escape(pb["name"] or "—"),
                     age=pb["age"] or "—", bio=html.escape(pb["bio"] or "—"))
            txt = (f"{t('match_final_title')}\n{info}\n\n"
                   f"{t('match_goto_hint')}")
            kb = match_goto_kb(ub, greeter)
            set_cur_lang(_sl)
            if pb["photo_id"]:
                await bot.send_photo(a, pb["photo_id"], caption=txt,
                                     reply_markup=kb)
            else:
                await bot.send_message(a, txt, reply_markup=kb)
            if not (ub and ub["username"]):
                _sl2 = cur_lang()
                set_cur_lang(get_lang(b))
                await bot.send_message(
                    b, t("auto_hello", name=html.escape(greeter)))
                set_cur_lang(_sl2)
        except TelegramError as e:
...
async def cb_like_yes(cq: CallbackQuery):
    ...
    try:
        ...
        return
    like = conn.execute(
        "SELECT * FROM nearby_likes WHERE id=?", (like_id,)).fetchone()
    if not like:
    ...
    try:
        ...
        conn.execute(
            "INSERT INTO nearby_likes (from_id, to_id, action, created_at) "
            "VALUES (?, ?, 'like', ?)", (uid, like["from_id"], now_iso()))
        conn.commit()
    except Exception as e:
...
async def cb_like_no(cq: CallbackQuery):
    try:
        ...
        pass
    await cq.answer(t("liked_ignored"))
# ---------- СОЗДАНИЕ / РЕДАКТУРОВАНИЕ АНКЕТЫ ----------
@dp.message(F.text, lambda m: (
    UD[m.from_user.id].get("state") or "").startswith("next_create_"))
...
async def next_create_photo(message: Message):
    ...
    try:
        existing = conn.execute(
            "SELECT 1 FROM nearby_profiles WHERE user_id=?", (uid,)).fetchone()
        if existing:
            conn.execute(
                "UPDATE nearby_profiles SET name=?, age=?, gender=?, "
                "looking_for=?, bio=?, photo_id=?, active=1, updated_at=? "
                "WHERE user_id=?",
                (d.get("name"), d.get("age"), d.get("gender"),
                 d.get("looking_for"), d.get("bio"), photo_id, now_iso(), uid))
        else:
            conn.execute(
                "INSERT INTO nearby_profiles "
                "(user_id, name, age, gender, looking_for, bio, photo_id, "
...
async def _next_relay(message, from_id, to_id, text=None, voice_file_id=None):
    ...
    try:
        ...
        await _reply(message, t("next_msg_failed"),
                     reply_markup=next_reaction_kb(from_id))
        return
    mid = None
    try:
        cur = conn.execute(
    ...
    if mid:
        try:
            await message.bot.edit_message_reply_markup(
                to_id, sent.message_id, reply_markup=next_reply_inline_kb(mid))
        except TelegramError:
            pass
    UD[from_id]["state"] = None
    await _reply(message, t("next_msg_sent"), reply_markup=next_reaction_kb(from_id))
...
async def cb_next_reply(cq: CallbackQuery):
    ...
    try:
        ...
        return
    msg = conn.execute(
        "SELECT * FROM nearby_messages WHERE id=?", (mid,)).fetchone()
    if not msg:
...
async def cb_next_report(cq: CallbackQuery):
    ...
    try:
        cur = conn.execute(
            "INSERT INTO reports "
            "(reporter_id, reported_id, context, reason, status, created_at) "
            "VALUES (?, ?, 'next_ls', NULL, 'pending', ?)",
            (uid, msg["from_id"], now_iso()))
        conn.commit()
        rid = cur.lastrowid or 0
    except Exception as e:
        log.warning("next ls report: %s", e)
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton(t("🔨 Бан"), callback_data=f"rep:ban:{rid}"),
        InlineKeyboardButton(t("❌ Отклонить"), callback_data=f"rep:no:{rid}"),
    ]])
...
===================== БЛОК 12 / 16 — АДМИНКА =====================
@dp.message(F.text.in_({"🛠 Админка", "🛠 Admin panel", "🛠 Admin"}))
...
@dp.message(F.text.in_({"🛡 Модерка", "🛡 Moderator", "🛡 Moderation"}))
...
@dp.message(F.text.in_({"📊 Статистика", "📊 Statistika", "📊 Statistics"}))
async def btn_stats(message: Message):
    uid = message.from_user.id
    if not is_staff(uid):
        return
    total_users = total_vip = total_msgs = 0
    total_refs = total_sessions = total_next = total_queue = 0
    try:
        row = conn.execute("SELECT COUNT(*) c FROM users").fetchone()
        if row:
            total_users = row["c"]
        ...
        if row:
            total_vip = row["c"]
        row = conn.execute("SELECT COUNT(*) c FROM anon_messages").fetchone()
        if row:
            total_msgs = row["c"]
        row = conn.execute(
            "SELECT COUNT(*) c FROM referrals WHERE active=1").fetchone()
        if row:
            total_refs = row["c"]
        row = conn.execute("SELECT COUNT(*) c FROM roulette_sessions").fetchone()
        if row:
            total_sessions = row["c"]
        row = conn.execute(
            "SELECT COUNT(*) c FROM nearby_profiles WHERE active=1").fetchone()
        if row:
            total_next = row["c"]
        row = conn.execute("SELECT COUNT(*) c FROM roulette_queue").fetchone()
        if row:
            total_queue = row["c"]
    except Exception:
        pass
    txt = (
        ...
        f"🎯 Анкет 𝐞𝐱𝐭..: <b>{total_next}</b>\n"
        f"⏳ Сейчас в поиске: <b>{total_queue}</b>"
    )
    kb = admin_menu_kb() if is_admin(uid) else moder_menu_kb()
    await _reply(message, txt, reply_markup=kb)
@dp.message(F.text.in_({"📤 Выгрузить пользователей",
                        "📤 Foydalanuvchilarni yuklash", "📤 Export users"}))
async def btn_export_users(message: Message):
    ...
    try:
        rows = conn.execute(
            "SELECT tg_id, username, first_name, gender, age, "
            "coins, vip_until, created_at FROM users").fetchall()
    except Exception:
        rows = []
    lines = ["tg_id | username | first_name | gender | age | "
             "coins | vip_until | created_at"]
    for r in rows:
        lines.append(
            f"{r['tg_id']} | {r['username'] or ''} | "
            f"{r['first_name'] or ''} | {r['gender'] or ''} | "
            f"{r['age'] or ''} | {r['coins'] or 0} | "
...
@dp.message(F.text.in_({"💰 Начислить коины", "💰 Coin qo'shish", "💰 Add coins"}))
...
@dp.message(F.text.in_({"👑 VIP по ID", "👑 ID bo'yicha VIP", "👑 VIP by ID"}))
...
@dp.message(F.text.in_({"➕ Выдать VIP", "➕ VIP berish", "➕ Grant VIP"}))
async def btn_vip_grant(message: Message):
    ...
    await _reply(message, t("vip_ask_id"), reply_markup=cancel_reply_kb())
@dp.message(F.text.in_({"➖ Забрать VIP", "➖ VIP olish", "➖ Revoke VIP"}))
async def btn_vip_revoke(message: Message):
    ...
    await _reply(message, t("vip_ask_id"), reply_markup=cancel_reply_kb())
@dp.message(F.text.in_({"👑 VIP всем", "👑 Hammaga VIP", "👑 VIP to all"}))
...
@dp.message(F.text.in_({"👑 VIP девушкам", "👑 Qizlarga VIP", "👑 VIP to girls"}))
...
@dp.message(F.text.in_({"👑 VIP парням", "👑 Yigitlarga VIP", "👑 VIP to guys"}))
...
async def btn_vip_bulk_grant(message: Message):
    ...
    names = {"all": "всем", "female": "девушкам", "male": "парням"}
    await _reply(message, t("vip_bulk_ask_days", target=names[bulk_filter]),
                 reply_markup=cancel_reply_kb())
@dp.message(F.text.in_({
    "➖ Забрать у всех", "➖ Hammadan olish", "➖ Revoke from all",
    "➖ Забрать у девушек", "➖ Qizlardan olish", "➖ Revoke from girls",
...
async def btn_vip_bulk_revoke(message: Message):
    ...
    try:
        ...
        conn.commit()
        row = conn.execute(f"SELECT COUNT(*) c FROM users {where}").fetchone()
        if row:
            cnt = row["c"]
    except Exception as e:
        log.warning("vip bulk revoke: %s", e)
    await _reply(message,
                 t("vip_bulk_revoke_done", target=names[bulk_filter], count=cnt),
                 reply_markup=admin_vip_kb())
@dp.message(F.text.in_({"🛡 Модеры", "🛡 Moderatorlar", "🛡 Moderators"}))
async def btn_moders_menu(message: Message):
    uid = message.from_user.id
    if not is_admin(uid):
        return
    await _reply(message, "🛡 <b>Управление модерами</b>",
                 reply_markup=admin_moder_kb())
@dp.message(F.text.in_({"➕ Выдать модера", "➕ Moder berish", "➕ Grant moder"}))
async def btn_moder_grant(message: Message):
    ...
    await _reply(message, t("vip_ask_id"), reply_markup=cancel_reply_kb())
@dp.message(F.text.in_({"➖ Забрать модера", "➖ Moderni olish", "➖ Revoke moder"}))
async def btn_moder_revoke(message: Message):
    ...
    await _reply(message, t("vip_ask_id"), reply_markup=cancel_reply_kb())
@dp.message(F.text.in_({"🔨 Бан / Разбан", "🔨 Ban / Unban"}))
async def btn_ban_menu(message: Message):
    uid = message.from_user.id
    if not is_staff(uid):
        return
    UD[uid]["state"] = "adm_ban_id"
    await _reply(message, "🔨 Введите ID пользователя:",
                 reply_markup=cancel_reply_kb())
@dp.message(F.text.in_({"📢 Рассылка", "📢 Xabar tarqatish", "📢 Broadcast"}))
async def btn_broadcast(message: Message):
    uid = message.from_user.id
    if not is_admin(uid):
        return
    UD[uid]["state"] = "adm_bcast_audience"
    await _reply(message, "📢 Выберите аудиторию:",
                 reply_markup=bcast_audience_kb())
...
async def bcast_all(message: Message):
    ...
    await _reply(message, "✉️ Отправьте сообщение:",
                 reply_markup=cancel_reply_kb())
...
async def bcast_send(message: Message):
    ...
    if flt == "male":
        ...
        where += " AND gender='f'"
    try:
        rows = conn.execute(f"SELECT tg_id FROM users {where}",
    ...
    await _reply(message, f"✅ Рассылка завершена\n✔️ {ok} | ❌ {bad}",
                 reply_markup=admin_menu_kb())
@dp.message(F.text.in_({"📢 Обязательные каналы", "📢 Majburiy kanallar",
                        "📢 Required channels"}))
async def btn_channels(message: Message):
    ...
    if not chans:
        ...
        for i, c in enumerate(chans, 1):
            lines.append(f"{i}. {channel_title(c)} — "
                         f"<code>{c['chat_username']}</code>")
        txt = "\n".join(lines)
    kb = adm_channels_kb(uid) if is_admin(uid) else moder_menu_kb()
    await _reply(message, txt, reply_markup=kb)
@dp.message(F.text.in_({"➕ Добавить канал", "➕ Kanal qo'shish", "➕ Add channel"}))
async def btn_add_channel(message: Message):
    ...
    await _reply(message, "📢 Отправьте @username канала или ссылку t.me/...",
                 reply_markup=cancel_reply_kb())
@dp.message(F.text.in_({"🗑 Удалить канал", "🗑 Kanalni o'chirish",
                        "🗑 Delete channel"}))
...
async def adm_channel_add_save(message: Message):
    ...
    try:
        ...
        await _reply(message, "❌ Ошибка добавления",
                     reply_markup=cancel_reply_kb())
...
async def adm_channel_del_save(message: Message):
    ...
    if not txt.isdigit():
        await _reply(message, "❌ Введите номер:",
                     reply_markup=cancel_reply_kb())
        return
    chans = channels_deletable_by(uid)
    idx = int(txt) - 1
    if idx < 0 or idx >= len(chans):
        await _reply(message, "❌ Нет такого номера.",
                     reply_markup=cancel_reply_kb())
        return
    cid = chans[idx]["id"]
...
async def toggle_subgate(message: Message):
    ...
    label = "ВКЛ" if new == "1" else "ВЫКЛ"
    await _reply(message, f"✅ Подписка: {label}",
                 reply_markup=adm_channels_kb(uid))
@dp.message(F.text.in_({"⭐ Коины за Stars", "⭐ Stars uchun coin",
                        "⭐ Coins for Stars"}))
async def btn_stars_admin(message: Message):
    ...
    for p in pkgs:
        ...
        lines.append(f"{status} #{p['id']} {p['title']} — "
                     f"{p['coins']}💰 / {p['price_stars']}⭐")
    body = "\n".join(lines) if lines else "Пусто"
    await _reply(message, body, reply_markup=star_admin_kb())
@dp.message(F.text.in_({"➕ Добавить пакет коинов", "➕ Coin paket qo'shish",
                        "➕ Add coin package"}))
async def btn_add_pkg(message: Message):
    ...
    await _reply(message, "📝 Введите название пакета:",
                 reply_markup=cancel_reply_kb())
@dp.message(F.text.in_({"🗑 Удалить пакет коинов", "🗑 Coin paketni o'chirish",
                        "🗑 Delete coin package"}))
async def btn_del_pkg(message: Message):
    ...
    await _reply(message, "🗑 Введите #ID пакета:",
                 reply_markup=cancel_reply_kb())
@dp.message(F.text.in_({"⭐ Возврат Stars", "⭐ Stars qaytarish",
                        "⭐ Refund Stars"}))
async def btn_refund_stars(message: Message):
    ...
    await _reply(message, "⭐ Введите ID пользователя для возврата:",
                 reply_markup=cancel_reply_kb())
@dp.message(F.text.in_({"💎 Цена раскрытия", "💎 Ochish narxi", "💎 Reveal price"}))
async def btn_reveal_price(message: Message):
    ...
    await _reply(message, f"💎 <b>Пакеты раскрытия:</b>\n{body}",
                 reply_markup=admin_menu_kb())
@dp.message(F.text, lambda m: (
    UD[m.from_user.id].get("state") or "").startswith("adm_"))
async def adm_state_router(message: Message):
    ...
    if state == "adm_coins_id":
        target = resolve_user_ref(text)
        if not target:
            await _reply(message, t("vip_user_not_found"),
                         reply_markup=cancel_reply_kb())
            return
        UD[uid]["adm_target"] = target
        UD[uid]["state"] = "adm_coins_amount"
        await _reply(message, "💰 Введите количество коинов:",
                     reply_markup=cancel_reply_kb())
        return
    if state == "adm_coins_amount":
        if not text.lstrip("-").isdigit():
            await _reply(message, "🔢 Введите число:",
                         reply_markup=cancel_reply_kb())
            return
        amt = int(text)
    ...
    if state == "adm_vip_grant_days":
        if not text.isdigit():
            await _reply(message, t("vip_days_number"),
                         reply_markup=cancel_reply_kb())
            return
        days = int(text)
        tgt = UD[uid]["adm_target"]
    ...
    if state == "adm_vip_revoke_id":
        ...
        conn.commit()
        UD[uid]["state"] = None
        await _reply(message, t("vip_taken_admin", id=target),
                     reply_markup=admin_vip_kb())
        try:
            _sl = cur_lang()
            set_cur_lang(get_lang(target))
    ...
    if state.startswith("adm_vip_bulk_days:"):
        ...
        names = {"all": "всем", "female": "девушкам", "male": "парням"}
        await _reply(message,
                     t("vip_bulk_done", target=names[flt], days=days, count=cnt),
                     reply_markup=admin_vip_kb())
        return
    if state == "adm_moder_grant_id":
        ...
        conn.commit()
        UD[uid]["state"] = None
        await _reply(message, f"✅ Модер выдан {target}",
                     reply_markup=admin_moder_kb())
        try:
            _sl = cur_lang()
            set_cur_lang(get_lang(target))
    ...
    if state == "adm_moder_revoke_id":
        ...
        if not target:
            await _reply(message, t("vip_user_not_found"),
                         reply_markup=cancel_reply_kb())
            return
        conn.execute("UPDATE users SET is_moder=0, moder_until=NULL "
                     "WHERE tg_id=?", (target,))
        conn.commit()
        UD[uid]["state"] = None
        await _reply(message, f"✅ Модер снят {target}",
                     reply_markup=admin_moder_kb())
        try:
            _sl = cur_lang()
            set_cur_lang(get_lang(target))
    ...
    if state == "adm_pkg_title":
        ...
        await _reply(message, "💰 Сколько коинов в пакете?",
                     reply_markup=cancel_reply_kb())
        return
    ...
    if state == "adm_pkg_stars":
        ...
        try:
            conn.execute(
                ...
                "VALUES (?, ?, ?, 1)", (p["title"], p["coins"], int(text)))
            conn.commit()
        except Exception as e:
    ...
    if state == "adm_pkg_del":
        ...
        if not check.isdigit():
            await _reply(message, "🔢 Введите ID:",
                         reply_markup=cancel_reply_kb())
            return
        pid = int(check)
        ...
        conn.commit()
        UD[uid]["state"] = None
        await _reply(message, f"✅ Удалён пакет #{pid}",
    ...
    if state == "adm_refund":
        ...
        try:
            ...
            for r in rows:
                conn.execute(
                    "UPDATE star_purchases SET refunded=1, refunded_at=? "
        ...
        await _reply(message, f"✅ Возврат выполнен. Всего звёзд: {refunded}",
                     reply_markup=admin_menu_kb())
        return
===================== БЛОК 13 / 16 — МАГАЗИН, STARS, PREMIUM, SEX, ANON =====================
T["sex_room_created"] = {
    "ru": "✅ <b>{title}</b> создана!\n📨 Приглашения разосланы — вход по согласию",
    "uz": "✅ <b>{title}</b> yaratildi!\n📨 Takliflar yuborildi",
    "en": "✅ <b>{title}</b> created!\n📨 Invites sent — join by consent",
}
T["sex_invite"] = {
    "ru": "💬 Комната: <b>{title}</b>\nВходишь по своему желанию:",
    "uz": "💬 Xona: <b>{title}</b>\nO'z xohishing bilan:",
    "en": "💬 Room: <b>{title}</b>\nJoin by your own choice:",
}
def _shop_kb(u):
    rows = []
...
async def btn_shop(message: Message):
    ...
    await _reply(message, t("shop_title"), reply_markup=_shop_kb(u))
...
async def shop_buy_handler(message: Message):
    ...
    u = get_user(uid)
    if " · " not in text:
        await _reply(message, t("shop_pick_item"), reply_markup=_shop_kb(u))
        return
    ...
    if rtype == "coins":
        ...
        try:
            ...
            await _reply(message, t("purchase_manual"),
                         reply_markup=main_menu_kb(uid))
            for adm in ADMIN_IDS:
                try:
                    await message.bot.send_message(
    ...
    try:
        conn.execute(
            ...
            "VALUES (?, ?, ?, ?)", (uid, target["id"], price, now_iso()))
        conn.commit()
    except Exception:
        pass
def _stars_kb():
    ...
    rows.append([KeyboardButton("⬅️ Назад")])
    return tr_kb(ReplyKeyboardMarkup(rows, resize_keyboard=True))
@dp.message(F.text.in_({"💎 Купить коины", "💎 Coin sotib olish", "💎 Buy coins"}))
async def btn_buy_coins(message: Message):
    ...
    await _reply(message, t("stars_title"), reply_markup=_stars_kb())
@dp.message(F.text, lambda m: UD[m.from_user.id].get("state") == "stars_browse")
async def stars_buy(message: Message):
    uid = message.from_user.id
    text = (message.text or "").strip()
    if " · " not in text or "💰" not in text:
        await _reply(message, t("stars_pick_pkg"), reply_markup=_stars_kb())
        return
    pkgs = conn.execute("SELECT * FROM star_packages WHERE active=1").fetchall()
    target = None
    for p in pkgs:
    ...
    try:
        await message.bot.send_invoice(
            chat_id=uid, title=item_title(target),
            description=t("stars_pkg_desc", coins=target["coins"]),
            payload=f"starpkg:{target['id']}", provider_token="",
            currency="XTR",
            prices=[_AiLP(label=item_title(target),
        ...
        await _reply(message, "❌ Ошибка выставления счёта",
                     reply_markup=main_menu_kb(uid))
@dp.message(F.text.in_({"⭐ Premium", "⭐ Premium", "⭐ Premium"}))
async def btn_premium(message: Message):
    ...
    if is_vip(u) and not is_admin(uid):
        try:
            dt = datetime.fromisoformat(u["vip_until"])
            await _reply(message, t("premium_active",
...
@dp.message(F.text.in_({"👁 Купить «Узнать»", "👁 «Aniqlash» sotib olish",
                        "👁 Buy «Reveal»"}))
...
async def reveal_buy_pack(message: Message):
    ...
    try:
        await message.bot.send_invoice(
            chat_id=uid, title=f"👁 Reveal × {pkg['count']}",
            description=t("reveal_desc"), payload=f"revpkg:{pkg['count']}",
            provider_token="", currency="XTR",
            prices=[_AiLP(label=f"Reveal × {pkg['count']}", amount=pkg["stars"])])
    except TelegramError as e:
        log.warning("reveal invoice: %s", e)
# ---------- SEX-КОМНАТЫ: вход по согласию ----------
def sex_room_kb(uid, room):
    rows = [[InlineKeyboardButton(tr_btn("👥 Участники"),
                                  callback_data=f"sexmembers:{room['id']}")]]
    is_owner = room["owner_id"] == uid
    if is_owner or is_admin(uid):
        rows.append([
            InlineKeyboardButton(tr_btn("✏️ Переименовать"),
                                 callback_data=f"sexrename:{room['id']}"),
            InlineKeyboardButton(tr_btn("🗑 Удалить комнату"),
                                 callback_data=f"sexdel:{room['id']}"),
        ])
    rows.append([InlineKeyboardButton(tr_btn("🚪 Запросить выход"),
                                      callback_data=f"sexexit:{room['id']}")])
    return InlineKeyboardMarkup(rows)
...
async def cmd_sex(message: Message):
    ...
    if room:
        await _reply(message, t("sex_room_exists",
                                title=html.escape(room["title"])),
                     reply_markup=sex_room_kb(uid, room))
        return
...
async def sex_create_title(message: Message):
    ...
    try:
        ...
        if not rid:
            row = conn.execute(
                "SELECT id FROM sex_rooms ORDER BY id DESC LIMIT 1").fetchone()
            if row:
                rid = row["id"]
        conn.execute(
            "INSERT INTO sex_members (room_id, user_id, role, number, joined_at) "
            "VALUES (?, ?, 'owner', 0, ?)", (rid, uid, now_iso()))
        conn.commit()
        girls = conn.execute(
            "SELECT tg_id FROM users WHERE gender='f'").fetchall()
        _sl = cur_lang()
        for g in girls:
            try:
                set_cur_lang(get_lang(g["tg_id"]))
                await message.bot.send_message(
                    g["tg_id"], t("sex_invite", title=html.escape(title)),
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton(tr_btn("💬 Войти в комнату"),
                                             callback_data=f"sexjoin:{rid}")]]))
            except TelegramError:
                pass
        set_cur_lang(_sl)
        UD[uid]["state"] = None
        room = conn.execute("SELECT * FROM sex_rooms WHERE id=?", (rid,)).fetchone()
...
def _sex_user_room(uid):
    try:
        return conn.execute(
            "SELECT * FROM sex_members WHERE user_id=?", (uid,)).fetchone()
    except Exception:
        return None
async def _sex_relay(message, uid):
    ...
    try:
        if message.voice:
            conn.execute(
                "INSERT INTO sex_messages (room_id, sender_number, content_type, "
                "voice_file_id, created_at) VALUES (?, ?, 'voice', ?, ?)",
    ...
    for o in others:
        try:
            ...
            pass
@dp.callback_query(F.data.startswith("sexjoin:"))
async def cb_sex_join(cq: CallbackQuery):
    uid = cq.from_user.id
    try:
    ...
    room = conn.execute("SELECT * FROM sex_rooms WHERE id=?", (rid,)).fetchone()
    if not room:
        await cq.answer(t("sex_no_room"), show_alert=True)
        return
    exists = conn.execute(
        "SELECT 1 FROM sex_members WHERE room_id=? AND user_id=?",
        (rid, uid)).fetchone()
    if exists:
        await cq.answer(t("sex_room_joined", title=room["title"]))
        return
    row = conn.execute(
        "SELECT MAX(number) m FROM sex_members WHERE room_id=?", (rid,)).fetchone()
    num = (row["m"] or 0) + 1
    try:
        conn.execute(
            "INSERT INTO sex_members (room_id, user_id, role, number, joined_at) "
            "VALUES (?, ?, 'member', ?, ?)", (rid, uid, num, now_iso()))
        conn.commit()
    except Exception:
        await cq.answer(t("sex_room_joined", title=room["title"]))
        return
    others = conn.execute(
        "SELECT user_id FROM sex_members WHERE room_id=? AND user_id<>?",
        (rid, uid)).fetchall()
    u = get_user(uid)
    nm = html.escape((u["first_name"] if u else None) or "—")
    for o in others:
        try:
            await cq.bot.send_message(
                o["user_id"], t("sex_member_joined_you", name=nm, num=num))
        except TelegramError:
            pass
    await cq.answer(t("sex_room_joined", title=room["title"]))
    try:
        await cq.bot.send_message(
            uid, t("sex_room_joined", title=html.escape(room["title"])),
            reply_markup=sex_room_kb(uid, room))
    except TelegramError:
        pass
@dp.callback_query(F.data.startswith("sexmembers:"))
async def cb_sex_members(cq: CallbackQuery):
    uid = cq.from_user.id
    mem = _sex_user_room(uid)
    if not mem and not is_staff(uid):
        await cq.answer("❌", show_alert=True)
        return
    try:
        rid = int(cq.data.split(":", 1)[1])
        ...
        return
    rows = conn.execute(
        "SELECT user_id, role, number FROM sex_members WHERE room_id=? "
        "ORDER BY number", (rid,)).fetchall()
    lines = ["👥 <b>Участники:</b>"]
    for r in rows:
        tag = "👑" if r["role"] == "owner" else f"#{r['number']}"
        lines.append(f"{tag} <code>{r['user_id']}</code>")
    await cq.answer("\n".join(lines), show_alert=True)
@dp.callback_query(F.data.startswith("sexexit:"))
async def cb_sex_exit_req(cq: CallbackQuery):
    uid = cq.from_user.id
    mem = _sex_user_room(uid)
    if not mem:
        await cq.answer(t("sex_no_room"), show_alert=True)
    ...
    rid = mem["room_id"]
    room = conn.execute("SELECT * FROM sex_rooms WHERE id=?", (rid,)).fetchone()
    if room and room["owner_id"] == uid:
        await cb_sex_delete_do(cq, rid)
        return
    try:
        conn.execute(
            "INSERT OR IGNORE INTO sex_exit_requests "
            "(room_id, user_id, status, created_at) VALUES (?, ?, 'pending', ?)",
            (rid, uid, now_iso()))
        conn.commit()
    except Exception:
        pass
    ...
    if room:
        staff_ids.add(room["owner_id"])
    for sid in staff_ids:
        try:
            await cq.bot.send_message(
                sid, f"🚪 Запрос выхода из комнаты #{rid} от <code>{uid}</code>",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton(tr_btn("🛡 Одобрить выход"),
                                         callback_data=f"sexexit_ok:{rid}:{uid}")]]))
        except TelegramError:
            pass
    await cq.answer(t("sex_exit_requested"))
async def cb_sex_delete_do(cq, rid):
    room = conn.execute("SELECT * FROM sex_rooms WHERE id=?", (rid,)).fetchone()
    if not room:
        await cq.answer(t("sex_no_room"), show_alert=True)
        return
    if room["owner_id"] != cq.from_user.id and not is_admin(cq.from_user.id):
        await cq.answer(t("sex_only_owner_can_delete"), show_alert=True)
        return
    members = conn.execute(
        "SELECT user_id FROM sex_members WHERE room_id=?", (rid,)).fetchall()
    conn.execute("DELETE FROM sex_members WHERE room_id=?", (rid,))
    conn.execute("DELETE FROM sex_messages WHERE room_id=?", (rid,))
    conn.execute("DELETE FROM sex_exit_requests WHERE room_id=?", (rid,))
    conn.execute("DELETE FROM sex_rooms WHERE id=?", (rid,))
    conn.commit()
    for m in members:
        try:
            await cq.bot.send_message(m["user_id"], t("sex_room_deleted"))
        except TelegramError:
            pass
    await cq.answer(t("sex_room_deleted"))
@dp.callback_query(F.data.startswith("sexexit_ok:"))
async def cb_sex_exit_ok(cq: CallbackQuery):
    uid = cq.from_user.id
    parts = cq.data.split(":")
    if len(parts) != 3:
        await cq.answer()
    ...
    try:
        ...
        return
    room = conn.execute("SELECT * FROM sex_rooms WHERE id=?", (rid,)).fetchone()
    if not room or (room["owner_id"] != uid and not is_admin(uid)):
        await cq.answer("❌", show_alert=True)
        return
    conn.execute("DELETE FROM sex_members WHERE room_id=? AND user_id=?",
                 (rid, target))
    conn.execute("DELETE FROM sex_exit_requests WHERE room_id=? AND user_id=?",
                 (rid, target))
    conn.commit()
    try:
        await cq.bot.send_message(target, t("sex_exit_approved"))
    except TelegramError:
        pass
    await cq.answer(t("done"))
@dp.callback_query(F.data.startswith("sexrename:"))
async def cb_sex_rename(cq: CallbackQuery):
    uid = cq.from_user.id
    try:
        rid = int(cq.data.split(":", 1)[1])
    except (ValueError, IndexError):
    ...
    room = conn.execute("SELECT * FROM sex_rooms WHERE id=?", (rid,)).fetchone()
    if not room or (room["owner_id"] != uid and not is_admin(uid)):
        await cq.answer(t("sex_only_owner_can_rename"), show_alert=True)
        return
    UD[uid]["state"] = f"sex_rename:{rid}"
    await cq.message.answer(t("sex_renamed_prompt"), reply_markup=cancel_reply_kb())
    await cq.answer()
@dp.message(F.text, lambda m: (
    UD[m.from_user.id].get("state") or "").startswith("sex_rename:"))
async def sex_rename_save(message: Message):
    uid = message.from_user.id
    rid = int(UD[uid]["state"].split(":", 1)[1])
    title = (message.text or "").strip()
    if not title:
        return
    conn.execute("UPDATE sex_rooms SET title=? WHERE id=?", (title, rid))
    conn.commit()
    UD[uid]["state"] = None
    room = conn.execute("SELECT * FROM sex_rooms WHERE id=?", (rid,)).fetchone()
    await _reply(message, t("sex_room_renamed", title=html.escape(title)),
                 reply_markup=sex_room_kb(uid, room))
@dp.callback_query(F.data.startswith("sexdel:"))
async def cb_sex_del(cq: CallbackQuery):
    try:
        rid = int(cq.data.split(":", 1)[1])
    except (ValueError, IndexError):
        await cq.answer()
...
async def anon_watch_handle(message: Message):
    ...
    msgs = conn.execute(
        ...
        "ORDER BY id DESC LIMIT 100", (target, target)).fetchall()
    if not msgs:
        kb = admin_menu_kb() if is_admin(uid) else moder_menu_kb()
    ...
    for m in reversed(msgs):
        lines.append(f"[{m['created_at']}] {m['from_id']} → {m['to_id']} "
                     f"({m['content_type']}): {m['text'] or ''}")
    data = "\n".join(lines).encode("utf-8")
    try:
        conn.execute(
            "INSERT INTO anon_watchers (mod_id, target_id, active, created_at) "
            "VALUES (?, ?, 1, ?)", (uid, target, now_iso()))
        conn.commit()
        await message.bot.send_document(
            uid, BufferedInputFile(data, filename=f"anon{target}.txt"),
...
async def cmd_tg_monitor(message: Message):
    ...
    try:
        row = conn.execute(
            "SELECT COUNT(*) c FROM roulette_sessions WHERE active=1").fetchone()
        if row:
            active = row["c"]
        row = conn.execute("SELECT COUNT(*) c FROM roulette_queue").fetchone()
        if row:
...
async def cb_anon_report(cq: CallbackQuery):
    ...
    try:
        cur = conn.execute(
            "INSERT INTO reports (reporter_id, reported_id, context, ref_id, "
            "status, created_at) VALUES (?, ?, 'anon', ?, 'pending', ?)",
    ...
    text200 = parent["text"][:200] if parent["text"] else ""
    for adm in ADMIN_IDS:
        try:
            await cq.bot.send_message(
                adm,
                f"🚩 <b>Жалоба #{rid}</b>\n👤 От: {rep_mention}\n"
                f"🎯 На: {from_mention}\n📝 Контекст: {html.escape(text200)}",
                reply_markup=kb)
        except TelegramError:
            pass
...
async def cb_anon_reveal(cq: CallbackQuery):
    ...
    try:
        await cq.bot.send_invoice(
            chat_id=uid, title="👁 Раскрытие отправителя",
            description=t("reveal_desc"), payload=f"reveal_anon:{pid}",
            provider_token="", currency="XTR",
            prices=[_AiLP(label="👁 Reveal", amount=1)])
    except TelegramError as e:
...
async def _do_reveal_anon(bot, uid, target_id):
    ...
    _sl = cur_lang()
    set_cur_lang(get_lang(uid))
    txt = t("reveal_result", name=html.escape(fu["first_name"] or "—"),
...
async def cb_ref_info(cq: CallbackQuery):
    await cq.answer(t("ref_info_alert", n=REF_REWARD_NORMAL, v=REF_REWARD_VIP),
                    show_alert=True)
...
async def cb_claim_vip(cq: CallbackQuery):
    ...
    claims = (u["ref_vip_claims"] or 0) if u else 0
    need = cfg_vip_threshold() * (claims + 1)
    if have < need:
    ...
    until = (now_dt() + timedelta(days=days)).isoformat()
    conn.execute("UPDATE users SET vip_until=? WHERE tg_id=?", (until, uid))
    conn.execute("UPDATE users SET ref_vip_claims = ref_vip_claims + 1 "
                 "WHERE tg_id=?", (uid,))
    conn.commit()
...
async def cb_claim_moder(cq: CallbackQuery):
    ...
    u = get_user(uid)
    have = qualified_referrals(uid)
    claims = (u["ref_moder_claims"] or 0) if u else 0
    need = cfg_moder_threshold() * (claims + 1)
    if have < need:
        await cq.answer(t("ref_need_more", n=need - have, have=have, need=need),
                        show_alert=True)
        return
    ...
    conn.execute("UPDATE users SET ref_moder_claims = ref_moder_claims + 1 "
                 "WHERE tg_id=?", (uid,))
    conn.commit()
...
async def cb_modapp(cq: CallbackQuery):
    ...
    if action == "ok":
        ...
        try:
            await cq.bot.send_message(
                app["user_id"], t("moder_rejected_user", coins=app["price_paid"]))
        except TelegramError:
            pass
    try:
        await cq.message.edit_reply_markup(reply_markup=None)
...
async def cb_report_decision(cq: CallbackQuery):
    ...
    if len(parts) != 3:
        return
    action, rid_str = parts[1], parts[2]
    try:
        rid = int(rid_str)
    except ValueError:
        return
    ...
    if action == "ban":
        ...
        try:
            ...
            await cq.bot.send_message(reported,
                                      t("you_were_banned", days=BAN_DAYS))
            set_cur_lang(_sl)
        except TelegramError:
            pass
    else:
...
async def on_successful_payment(message: Message):
    ...
    if payload.startswith("starpkg:"):
        ...
        try:
            conn.execute(
                "INSERT INTO star_purchases (user_id, package_id, coins, stars, "
                "charge_id, created_at) VALUES (?, ?, ?, ?, ?, ?)",
        ...
        try:
            conn.execute(
                "INSERT INTO reveal_purchases (user_id, count, stars, charge_id, created_at) "
                "VALUES (?, ?, ?, ?, ?)",
                (uid, cnt, sp.total_amount, charge_id, now_iso()))
            conn.commit()
        except Exception as e:
            log.warning("revpkg: %s", e)
        add_reveal_paid(uid, cnt)
        ...
        try:
            ...
            await _reply(message, "❌", reply_markup=main_menu_kb(uid))
            return
        await _do_reveal_next(message.bot, uid, tid)
        await _reply(message, t("done"), reply_markup=main_menu_kb(uid))
    else:
        await _reply(message, t("done"), reply_markup=main_menu_kb(uid))
@dp.my_chat_member()
...
===================== БЛОК 15 / 16 — РОУТЕР, ФОН, HEALTH =====================
# handle_text_router регистрируем ПОСЛЕДНИМ: он ловит всё, что не поймали
# узкие фильтры выше (кнопки меню, стейты, глобальная отмена и т.д.)
dp.message.register(
    handle_text_router,
...
def start_web_server():
    ...
    try:
        ...
        t_web = threading.Thread(target=server.serve_forever, daemon=True)
        t_web.start()
        boot.info("Health-check сервер на порту %d", port)
    except Exception as e:
        boot.warning("web server: %s", e)
===================== БЛОК 16 / 16 — ДЕФОЛТЫ, КОМАНДЫ, STARTUP, MAIN =====================
def init_default_data():
    try:
        has_pkg = conn.execute("SELECT 1 FROM star_packages LIMIT 1").fetchone()
        if not has_pkg:
            packs = [
                ("50 коинов", "50 coin", "50 coins", 50, 50),
                ("120 коинов", "120 coin", "120 coins", 120, 100),
                ("300 коинов", "300 coin", "300 coins", 300, 200),
                ("700 коинов", "700 coin", "700 coins", 700, 400),
                ("1500 коинов", "1500 coin", "1500 coins", 1500, 750),
            ]
            for ttl_ru, ttl_uz, ttl_en, coins, stars in packs:
                conn.execute(
                    "INSERT INTO star_packages "
                    "(title, title_uz, title_en, coins, price_stars, active) "
                    "VALUES (?, ?, ?, ?, ?, 1)",
                    (ttl_ru, ttl_uz, ttl_en, coins, stars))
            conn.commit()
            log.info("Добавлены стартовые пакеты коинов")
        has_item = conn.execute("SELECT 1 FROM shop_items LIMIT 1").fetchone()
        if not has_item:
            items = [
                ("VIP на 7 дней", "7 kun VIP", "VIP 7 days",
                 "Премиум-статус на 7 дней", 300, 1, 7, "vip", None),
                ("VIP на 30 дней", "30 kun VIP", "VIP 30 days",
                 "Премиум-статус на 30 дней", 1000, 1, 30, "vip", None),
                ("100 коинов", "100 coin", "100 coins",
                 "Пополнение баланса", 150, 0, None, "coins", 100),
                ("Заявка на модера", "Moder arizasi", "Moder application",
                 "Подать заявку на роль модератора", 500, 0, None, "manual", None),
            ]
            for ttl, ttl_uz, ttl_en, dsc, price, is_vip, days, rt, amt in items:
                conn.execute(
                    "INSERT INTO shop_items "
                    "(title, title_uz, title_en, description, price, is_vip, "
                    "duration_days, reward_type, reward_amount, active) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)",
                    (ttl, ttl_uz, ttl_en, dsc, price, is_vip, days, rt, amt))
            conn.commit()
...
async def set_bot_commands():
    try:
        ...
        await bot.set_my_commands(cmds)
        boot.info("Bot commands set: only /start")
    except Exception as e:
        boot.warning("set_my_commands: %s", e)
...
async def _run_async():
    ...
    try:
        ...
        await on_shutdown()
def main():
    """Точка входа: совместимо с main.py: from bot import main; main()."""
    try:
